...
\section{Kleene Algebra and Kozen Categories}
...
structured \emph{poset}. We then generalize this to a Kozen
...
\begin{definition}
  ...
  a \textbf{parameterized $A$-cons-list object} consists of
  \begin{enumerate}
...
\end{tikzcd}\]
  ...
  An \textbf{$A$-cons-list object} is like a parameterized cons-list object but where $P$ can only be $I$.

  A \textbf{parameterized $A$-snoc-list object} is an $A$-cons-list object in
...
\begin{construction}
  Given an $A$-cons-list object, we can define a morphism $\textrm{snoc} : L \otimes A \to L$ as $\textrm{foldr}(\eta, \textrm{cons})$
...
Here's a very concise formulation:
\begin{definition}
  An \textbf{$A$-$B$-cons-list object} is an initial algebra for the functor $\textrm{ConsF}({A,B})(X) = B + A \otimes X$.
  An \textbf{$A$-$B$-snoc-list object} is an initial algebra for the functor $\textrm{SnocF}({A,B})(X) = B + X \otimes A$.

  A \textbf{$A$-$B$-cons-list object} $c : B + A \otimes L \to L$ is
  \emph{distributive} when for every $C$, the following induced
  $\textrm{ConsF}({A,B\otimes C})$-algebra on $L \otimes C$ is
  initial:
% https://q.uiver.app/#q=WzAsMyxbMCwwLCIoQlxcb3RpbWVzIEMpK0FcXG90aW1lcyAoTFxcb3RpbWVzIEMpIl0sWzEsMCwiKEIrQVxcb3RpbWVzIEwpXFxvdGltZXMgQyJdLFsyLDAsIkwgXFxvdGltZXMgQyJdLFswLDEsIlxcZGVsdGEiXSxbMSwyLCJjIFxcb3RpbWVzIEMiXV0=
  \[\begin{tikzcd}
	{(B\otimes C)+A\otimes (L\otimes C)} & {(B+A\otimes L)\otimes C} & {L \otimes C}
	\arrow["\delta", from=1-1, to=1-2]
	\arrow["{c \otimes C}", from=1-2, to=1-3]
  \end{tikzcd}\]
  where $\delta$ is the obvious distribution isomorphism.

  A distributive initial $\textrm{ConsF}(A,I)$ algebra on $L$ induces
  a $\textrm{SnocF}(A,B)$-algebra on $L$ and vice-versa.
\end{definition}

\begin{lemma}
  A parameterized $A$-cons list object is equivalent to a distributive
  $A$-$I$-cons list object.
\end{lemma}

\begin{definition}
  An {A}-dequeue object is an object $L$ equipped with the
  structure of a distributive initial $\textrm{ConsF}(A,I)$ algebra $c
  : I \oplus A\otimes L \to L$ and distributive initial
  $\textrm{SnocF}(A,I)$ algebra $s : I \oplus L \otimes I \to L$ such
  that the induced $\textrm{SnocF}(A,I)$ structure $s'$ from the cons
  structure is equal to $s$ and vice-versa the induced cons algebra
  $c'$ is equal to $c$.

  Equivalently, an $A$-dequeue object is a distributive initial
  $\textrm{ConsF}(A,I)$ algebra $c : I \oplus A\otimes L \to L$ such
  that the induced $\textrm{SnocF}(A,I)$ algebra $s : I + L \otimes A
  \to L$ is distributive initial.
\end{definition}

\begin{definition}
  A Kozen category is a distributive monoidal category $\cat{K}$
  with for every object $A$, an $A$-dequeue object $A^*$.
\end{definition}
...
\begin{lemma}
  A Kleene algebra is equivalent to a posetal Kozen category.
\end{lemma}

% This isn't an example of a Kozen category, it's an example of a model of conjunctive ordered logic
An unexpected example comes from the theory of substructural logics.

\begin{example}
  The opposite category of every Kozen category is a model of a variant of
...
Kleene star as a parameterized list and snoc object. If the Kozen
category $\cat{K}$ has more structure, then these isomorphisms hold
``for free''.

\begin{definition}
  A pre-Kozen category is a distributive monoidal category with a (non-parameterized) cons-list object.
\end{definition}

\begin{theorem}
  If $\cat K$ is a pre-Kozen category that is also biclosed, then it is a Kozen category.
\end{theorem}
\begin{proof}
...
%%   Let $\cat{K}$ be a Kozen category such that it is also monoidal
...
\begin{definition}
  Let $K$ be a Kozen category over alphabet $\Sigma$. This
  induces a strong monoidal functor $\Sigma^* \to K$. The
  nerve functor $w \mapsto K(w, -)$ is the grammar interpretation
  functor of $K$.
\end{definition}

\section{Proposed construction for free Kozen categories}
...
define the free Kozen category $\FKC(\cat{C})$. We can do that by
using the internal language of Kozen categories. Concretely, it
...
\begin{theorem}
  ...
  $\textrm{Reg}$ in $\cat{2Cat}$.
\end{theorem}
We use the notation $\textrm{Reg}(\cat C)$ for the free Kozen category
generated from $\cat C$ as its objects are the ``regular types''
generated from $\cat C$. When $\Sigma$ is a (discrete, finite) set
representing an alphabet we write $\textrm{Reg}(\Sigma)$ to mean the
free Kozen category generated by the discrete category $\Sigma$. In
this case, the objects of $\textrm{Reg}(\Sigma)$ are regular
expressions over the alphabet $\Sigma$ and the morphisms are
``primitive recursive parse transformers''.
...
the state $X$ is related to $Y$. This can be seen as a
...
related. We now formalize this intuition

\begin{example}[Automata with simulation relation]
  Various notions of simulation for automata can be formulated using
  the language of fibered category theory and liftings of functors.
  Assume that we have a determinisitc automaton over a state space $S$
  with a simulation relation $\mathcal{R}$. We can see $S$ and $\mathcal{R}$
  as a thin category and equip it with a canonical $F$-coalgebra structure.
  \begin{itemize}
  \item The component $(S, \mathcal{R}) \to \Set$ maps an element of $S$ to the
    singleton set $\{*\}$ if it is an accepting state and to the empty set
    otherwise. The action on morphisms is defined using the simulation
    acceptance condition, i.e. if $(x, y) \in \mathcal{R}$, then
    $\alpha(x) \leq \alpha(y)$. Functoriality follows from initiality/terminality
    of the empty and singleton sets, respectively.
  \item The component $(S, \mathcal{R}) \times \Sigma \to (S, \mathcal{R})$
    is, as expected, the transition function $\delta : S \times \Sigma \to S$.
    The action on morphisms is given by the transition condition of simulation
    relations, i.e. if $(x, y) \in \mathcal{R})$ then for every $c \in \Sigma$
    $(\delta(x, c), \delta(y, c)) \in \mathcal{R}$.
  \end{itemize}
\end{example}
...
The proof that this is a Kleene algebra is boring and unsurprising.
...
the Kozen category case. First, we have to define a natural
...
\times (-)^\Sigma) \to \Set$ is by equipping $\Set$ with a Kozen
category structure and then using the universal property of the free
Kozen category. The Kozen category structure on $\Set$ is the usual
...
we will once again use the universal property of free Kozen
...
\begin{lemma}
  Let $\cat{K}$ and $\cat{L}$ be two Kozen categories, $F : \cat{L} \to \cat{K}$
  and $G : \cat{L} \to \cat{K}$ two Kozen functors.
  We can equip $\cat{K} \times \cat{L}$ with the following Kozen
  category structure:
  \begin{itemize}
...
Kozen category structure given by the lemma above.

  In our case, we want to instantiate $F$ with the unit for the reader
  monad $(-)^\Sigma$. The generalization of $G$ is a bit subtle, as it
  is not enough to simply output weather a state accepts the empty
  string or not, we have to carry around the the witnesses of such
  parse trees. Usin the fact that $2$ is the free Kleene algebra with
  zero generators on the empty alphabet, we instantiate $G$ to be a
  ``prunning'' operation that factors through the free Kozen category
  on the empty category. More concretely, we define a functor
  $K(\cat{C}) \to K(\cat{C})^\Sigma$ by first defining a functor
  $K(\cat{C}) \to K(\emptyset)$ by using the universal property of free
  Kozen categories and mapping $\cat{C}$ to the constant $0$ object of
  $K(\emptyset)$. The functor $K(\emptyset) \to K(\cat{C})^\Sigma$ is defined
  using the fact that left adjoints preserve colimits, so $K(\emptyset)$
  is initial in the category of Kozen categories.
...
\section{Canonicity for Free Kozen Categories}

Let $\Sigma$ be a set. Then the ``standard semantics'' of the free
Kozen category $\FKC(\Sigma)$ is the unique functor $\sem{\cdot} :
\FKC(\Sigma) \to \Set^{\Sigma^*}$ preserving the (Day convolution)
Kozen category structure.

We first address whether we can generalize Kozen's completeness
theorems for Kleene algebras. Firstly, the fact that the Free Kozen
...
algebra is equivalent to the posetal reflection of the free Kozen
...
of a morphism in the free Kozen category. But consider that the
...
syntax of the free Kozen category, which fails as all such functions
...
the \emph{equational theory} of free Kozen categories is
...
proof using the universal property of the free Kozen category.  To do
this we need to develop a Kozen category for which constructing a
...
\begin{theorem}[Gluing for Kozen categories]
  Let $\cat K, \cat L, \cat M$ be Kozen categories and $\Phi : \cat K
  \to \cat M$ be a Kozen functor and $G : \cat L \to \cat M$ be a lax
  monoidal functor. Then $\Phi \downarrow G$ is a Kozen category, and
  the projections are (strict) Kozen functors.
\end{theorem}
\begin{proof}
  The monoidal and coproduct aspects of Kozen categories are handled
...
Kozen functor and $N : \FKC(\Sigma) \to \Set^{\Sigma^*}$ is a lax
monoidal functor, and so $\sem{\cdot}\downarrow N$ is a Kozen
category and the projection to $\FKC(\Sigma)^2$ is a strict Kozen
functor. Then by the universal property of $\FKC(\Sigma)$, there is a
unique strict Kozen functor $q$ from $\FKC(\Sigma)$ to
$\sem{\cdot}\downarrow N$ and further the composition with the
projection to $\FKC(\Sigma)^2$ is strict Kozen and so $q$ is
...
\begin{proof}
  ...
  that $u \circ q$ and $\id$ correspond to (strict?) Kozen functors
  from $\FKC(\Sigma)$ to $\sem{\cdot} \downarrow \sem{\cdot}$. The
  identity part should be a general theorem, and verifying the Kozen

...
-}
module Erased.Relation.Nullary.Properties where
...
open import Erased.Data.Empty as ⊥
open import Erased.Data.Sigma.Base using (_×_)
...
@0 isoPresDiscrete : Iso A B → Discrete A → Discrete B
isoPresDiscrete e = sectionDiscrete fun inv rightInv
  where open Iso e

@0 EquivPresDiscrete : ∀ {ℓ ℓ'}{A : Type ℓ} {B : Type ℓ'} → A ≃ B
...
@0 Stable× : Stable A -> Stable B -> Stable (A × B)
Stable× As Bs e = λ where
  .fst → As λ k → e (k ∘ fst)
  .snd → Bs λ k → e (k ∘ snd)

@0 StableΣ : ∀ {A : Type ℓ} {P : A → Type ℓ'} →
...
@0 discreteDec : (Adis : Discrete A) → Discrete (Dec A)
...
@0 isPropDec : (Aprop : isProp A) → isProp (Dec A)
...
@0 EquivPresDec : ∀ {ℓ ℓ'}{A : Type ℓ} {B : Type ℓ'} → A ≃ B
          → Dec A → Dec B
...
@0 notEmptyPopulated : ⟪ A ⟫ → NonEmpty A
...
@0 Dec→Stable : Dec A → Stable A
Dec→Stable (yes x) = λ _ → x
Dec→Stable (no x) = λ f → ⊥.elim (f x)

@0 Stable→PStable : Stable A → PStable A
Stable→PStable st = st ∘ notEmptyPopulated

@0 PStable→SplitSupport : PStable A → SplitSupport A
...
@0 SplitSupport→Collapsible : SplitSupport A → Collapsible A
...
@0 Collapsible→SplitSupport : Collapsible A → SplitSupport A
Collapsible→SplitSupport f x = fixpoint (populatedBy x f)

@0 HSeparated→Collapsible≡ : HSeparated A → Collapsible≡ A
HSeparated→Collapsible≡ hst x y = SplitSupport→Collapsible (hst x y)

@0 Collapsible≡→HSeparated : Collapsible≡ A → HSeparated A
...
@0 Collapsible≡→isSet : Collapsible≡ A → isSet A
...
@0 HSeparated→isSet : HSeparated A → isSet A
HSeparated→isSet = Collapsible≡→isSet ∘ HSeparated→Collapsible≡

@0 isSet→HSeparated : isSet A → HSeparated A
...
@0 PStable≡→HSeparated : PStable≡ A → HSeparated A
PStable≡→HSeparated pst x y = PStable→SplitSupport (pst x y)

@0 PStable≡→isSet : PStable≡ A → isSet A
PStable≡→isSet = HSeparated→isSet ∘ PStable≡→HSeparated

@0 Separated→PStable≡ : Separated A → PStable≡ A
Separated→PStable≡ st x y = Stable→PStable (st x y)

@0 Separated→isSet : Separated A → isSet A
Separated→isSet = PStable≡→isSet ∘ Separated→PStable≡

@0 SeparatedΠ : (∀ x → Separated (P x)) -> Separated ((x : A) -> P x)
SeparatedΠ Ps f g e i x = Ps x (f x) (g x) (λ k → e (k ∘ cong (λ f → f x))) i

@0 Separated→ : Separated B -> Separated (A → B)
Separated→ Bs = SeparatedΠ (λ _ → Bs)

@0 Separated× : Separated A -> Separated B -> Separated (A × B)
...
@0 Discrete→Separated : Discrete A → Separated A
Discrete→Separated d x y = Dec→Stable (d x y)

@0 Discrete→isSet : Discrete A → isSet A
Discrete→isSet = Separated→isSet ∘ Discrete→Separated

@0 ≡no : ∀ {A : Type ℓ} x y → Path (Dec A) x (no y)
≡no (yes p) y = ⊥.rec (y p)
≡no (no ¬p) y i = no (isProp¬ _ ¬p y i)

@0 inhabitedFibres? : ∀ {ℓ'} {A : Type ℓ} {B : Type ℓ'}

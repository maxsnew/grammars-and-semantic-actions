...
@0 doubleCompPath-filler : (p : x ≡ y) (q : y ≡ z) (r : z ≡ w)
...
@0 compPath-unique : (p : x ≡ y) (q : y ≡ z) (r : z ≡ w)
...
@0 compPath-filler : (p : x ≡ y) (q : y ≡ z) → PathP (λ j → x ≡ q j) p (p ∙ q)
...
@0 compPath'-filler : (p : x ≡ y) (q : y ≡ z) → PathP (λ j → p (~ j) ≡ z) q (p ∙' q)
...
@0 compPath≡compPath' : (p : x ≡ y) (q : y ≡ z) → p ∙ q ≡ p ∙' q
...
@0 doubleCompPath≡compPath : {x y z w : A}
    (p : x ≡ y) (q : y ≡ z) (r : z ≡ w)
  → p ∙∙ q ∙∙ r ≡ p ∙ q ∙ r
...
@0 compPathP : {A : I → Type ℓ} {x : A i0} {y : A i1} {B_i1 : Type ℓ} {B : (A i1) ≡ B_i1} {z : B i1}
...
@0 compPathP' : {B : A → Type ℓ'} {x' : B x} {y' : B y} {z' : B z} {p : x ≡ y} {q : y ≡ z}
...
@0 compPathP-filler : {A : I → Type ℓ} {x : A i0} {y : A i1} {B_i1 : Type ℓ} {B : A i1 ≡ B_i1} {z : B i1}
...
@0 compPathP'-filler : {B : A → Type ℓ'} {x' : B x} {y' : B y} {z' : B z} {p : x ≡ y} {q : y ≡ z}
  (P : PathP (λ i → B (p i)) x' y') (Q : PathP (λ i → B (q i)) y' z')
...
@0 isGroupoid : Type ℓ → Type ℓ
isGroupoid A = ∀ a b → isSet (Path A a b)

@0 is2Groupoid : Type ℓ → Type ℓ
is2Groupoid A = ∀ a b → isGroupoid (Path A a b)

-- Contractibility of singletons

@0 singlP : (A : I → Type ℓ) (a : A i0) → Type _
...
@0 isContrSinglP : (A : I → Type ℓ) (a : A i0) → isContr (singlP A a)
...
@0 isPropSinglP : {A : I → Type ℓ} {a : A i0} → isProp (singlP A a)
...
infixr 4 _,_
Σ0-syntax : ∀ {ℓ ℓ'} (A : Type ℓ) (@0 B : A → Type ℓ') → Type (ℓ-max ℓ ℓ')
Σ0-syntax = Σ0

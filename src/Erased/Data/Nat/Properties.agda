{-# OPTIONS --no-exact-split -WnoUnsupportedIndexedMatch #-}
module Erased.Data.Nat.Properties where
-- TODO this file is janky and should be reorganized, because
-- I'm not even erasing the cubical glue. I'm just exporting some Data.Eq stuff
...
znots : ¬ (0 Eq.≡ suc n)
znots {n} ()
-- subst (caseNat ℕ ⊥) eq 0

snotz : ¬ (suc n Eq.≡ 0)
snotz ()

injSuc : suc m Eq.≡ suc n → m Eq.≡ n
injSuc {m} {n} Eq.refl = Eq.refl
...
-- @0 compute-eqℕ : (n m : ℕ) → (n ≡ m ) → codeℕ n m
...
discreteℕ : DiscreteEq ℕ
discreteℕ zero zero = yes Eq.refl
discreteℕ zero (suc m) = no λ ()
discreteℕ (suc n) zero = no λ ()
discreteℕ (suc n) (suc m) with discreteℕ n m
... | yes p = yes (Eq.ap suc p)

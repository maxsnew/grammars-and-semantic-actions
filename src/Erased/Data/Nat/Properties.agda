{-# OPTIONS --no-exact-split --erased-cubical #-}
...
@0 minComm : (n m : ℕ) → min n m ≡ min m n
...
@0 maxComm : (n m : ℕ) → max n m ≡ max m n
...
decEqℕ : DecEq ℕ
decEqℕ zero zero = yes Eq.refl
decEqℕ zero (suc b) = no (λ ())
decEqℕ (suc a) zero = no (λ ())
decEqℕ (suc a) (suc b) with decEqℕ a b

...
-}
module Erased.Data.Sigma.Properties where

open import Erased.Foundations.Prelude
open import Erased.Foundations.Function
...
open import Cubical.Data.Sigma.Base hiding (Σ-syntax)
...
@0 erasureΣIso : Iso (Σ[ a ∈ A ] B a) (Σ0[ a ∈ A ] B a)
erasureΣIso .fun x = x .fst , x .snd
erasureΣIso .inv x = x .fst , x .snd
erasureΣIso .rightInv _ = refl
erasureΣIso .leftInv _ = refl
...
module @0 _ {A : I → Type ℓ} {B : (i : I) → A i → Type ℓ'}
  ...
  module @0 PathPΣ (p : PathP (λ i → Σ (A i) (B i)) x y) where
...
@0 uniqueExists : (a : A) (b : B a) (h : (a' : A) → isProp (B a')) (H : (a' : A) → B a' → a ≡ a') → ∃![ a ∈ A ] B a
...
module @0 _ {A : I → Type ℓ} {B : (i : I) → (a : A i) → Type ℓ'}
  {x : Σ (A i0) (B i0)} {y : Σ (A i1) (B i1)}
  where
...
@0 discreteΣ : Discrete A → ((a : A) → Discrete (B a)) → Discrete (Σ A B)
...
@0 rUnit*×Iso : ∀{ℓ} → Iso (A × Unit* {ℓ}) A
fun rUnit*×Iso = fst
inv rUnit*×Iso = _, tt*
rightInv rUnit*×Iso _ = refl
leftInv rUnit*×Iso _ = refl

module @0 _ {A : Type ℓ} {A' : Type ℓ'} where
...
module @0 _ {A : Type ℓ} {B : A → Type ℓ'} {C : ∀ a → B a → Type ℓ''} where
...
module @0 _ {A : Type ℓ} {B : A → Type ℓ'} {B' : ∀ a → Type ℓ''} where
...
@0 Σ-cong-iso-fst : (isom : Iso A A') → Iso (Σ A (B ∘ fun isom)) (Σ A' B)
...
@0 Σ-cong-equiv-fst : (e : A ≃ A') → Σ A (B ∘ equivFun e) ≃ Σ A' B
...
@0 Σ-cong-iso-snd : ((x : A) → Iso (B x) (B' x)) → Iso (Σ A B) (Σ A B')
...
@0 Σ-cong-equiv-snd : (∀ a → B a ≃ B' a) → Σ A B ≃ Σ A B'
...
@0 IsoΣPathTransportPathΣ : (a b : Σ A B) → Iso (ΣPathTransport a b) (a ≡ b)
...
@0 PathΣ→ΣPathTransport : (a b : Σ A B) → (a ≡ b) → ΣPathTransport a b
...
@0 Σ-contractFst : (c : isContr A) → Σ A B ≃ B (c .fst)
Σ-contractFst {B = B} c = isoToEquiv (Σ-contractFstIso c)

-- a special case of the above
module @0 _ (A : Unit → Type ℓ) where
  ΣUnit : Σ Unit A ≃ A tt
  unquoteDef ΣUnit = defStrictEquiv {B = A tt} ΣUnit snd (tt ,_)

@0 Σ-contractSnd : ((a : A) → isContr (B a)) → Σ A B ≃ A
...
@0 isEmbeddingFstΣProp : ((x : A) → isProp (B x))
                    → {u v : Σ A B}
...
@0 Σ≡PropEquiv : ((x : A) → isProp (B x)) → {u v : Σ A B}
            → (u .fst ≡ v .fst) ≃ (u ≡ v)
Σ≡PropEquiv pB = invEquiv (_ , isEmbeddingFstΣProp pB)

@0 Σ≡Prop : ((x : A) → isProp (B x)) → {u v : Σ A B}
       → (p : u .fst ≡ v .fst) → u ≡ v
...
@0 discreteΣProp : Discrete A → ((x : A) → isProp (B x)) → Discrete (Σ A B)
discreteΣProp _≟_ isPropA _ _ =
  EquivPresDec (Σ≡PropEquiv isPropA) (_ ≟ _)

@0 ≃-× : ∀ {ℓ'' ℓ'''} {A : Type ℓ} {B : Type ℓ'} {C : Type ℓ''} {D : Type ℓ'''} → A ≃ C → B ≃ D → A × B ≃ C × D
≃-× eq1 eq2 =
    map-× (fst eq1) (fst eq2)
  , record
     { equiv-proof
...
@0 prodIso : ∀ {ℓ ℓ' ℓ'' ℓ'''} {A : Type ℓ} {B : Type ℓ'} {C : Type ℓ''} {D : Type ℓ'''}
       → Iso A C
       → Iso B D
       → Iso (A × B) (C × D)
...
@0 prodEquivToIso : ∀ {ℓ'' ℓ'''} {A : Type ℓ} {B : Type ℓ'} {C : Type ℓ''} {D : Type ℓ'''}
  → (e : A ≃ C)(e' : B ≃ D)
...
@0 separatedΣ : Separated A → ((a : A) → Separated (B a)) → Separated (Σ A B)

...
module _ {{monStr : MonoidalStr}} where
  ...
  module _ (A : Grammar ℓA) where
    ...
    *Ty _ = ⊕e *Tag (λ { nil → k ε* ; cons → (k A) ⊗e (Var _)})
    ...
    --       (grade
    --       ∘g roll ∘g {!!}
    --       -- ∘g ungrade' (lift (suc n))
    --       -- ∘g roll
    --       ∘g id ,⊗ (liftG ∘g eq-π _ _ ∘g lowerG)
    --          ≡⟨ {!!} ⟩
    --        {!!}
    --          ≡⟨ {!!} ⟩
    --       σ (lift (suc n))
    ...
    --       -- (λ i →
    --       --   ⊕ᴰ-elim {!λ (lift m) → σ (lift (suc m)) ∘g roll ∘g ?!}
    --       --   ∘g ⊕ᴰ-distR .fun
    --       --   ∘g id ,⊗ eq-π-pf _ _ i
    --       --   ∘g lowerG ,⊗ lowerG
    --       -- )
    ...
    -- secAlg (lift (suc n)) = the-sec-alg-snd n -- the-sec-alg-snd n
    ...
    -- *continuous .sec =
    --   ⊕ᴰ≡ _ _ λ n →
    --     equalizer-ind repeatTy _
    --       (λ n → grade ∘g ungrade' n)
    --       (λ n → σ n)
    --       (λ @0 where
    --         (lift zero) → refl
    --         (lift (suc n)) →
    --           grade
    --           ∘g roll ∘g σ cons
    --           ∘g id ,⊗ (liftG ∘g ungrade' (lift n) ∘g lowerG)
    --           ∘g id ,⊗ (liftG ∘g eq-π _ _ ∘g lowerG)
    --              ≡⟨ {!!} ⟩
    --            {!!}
    --              ≡⟨ {!!} ⟩

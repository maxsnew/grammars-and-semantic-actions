...
module Grammar.Inductive.AsEquality.Properties (Alphabet : Type ℓ-zero) (@0 isSetAlphabet : isSet Alphabet) where
...
module _ {ℓA : Level} where
  @0 ⟦_⟧≡ : {X : Type ℓX} → (F : Functor X) → IndPath.⟦ F ⟧ {ℓA = ℓA} ≡ ⟦ F ⟧
  ⟦ k A ⟧≡  = refl
  ⟦ Var x ⟧≡ = refl
...
module _ {ℓA : Level} {X : Type ℓX} (F : X → Functor X) where

...
module Grammar.Inductive.Liftless.Indexed (Alphabet : hSet ℓ-zero)where
...
open import Grammar.Inductive.Liftless.Functor Alphabet public
...
module _ where
  ...
  module _ {X : Type ℓX} (F : X → Functor X) where
    ...
    module _ {A} (α : Algebra F A) where
      {-# TERMINATING #-}

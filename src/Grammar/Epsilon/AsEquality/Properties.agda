...
open import Cubical.Foundations.Prelude hiding (Lift ; lower ; lift ; liftExt)
...
opaque
  ...
  isLangε*≡ : ∀ i → isLang (ε*≡ {ℓ = ℓ} i)
  isLangε*≡ i = subst-filler isLang ε*≡ εPath.isLangε* i

  ε-intro≡ : PathP (λ i → ε≡ i []) εPath.ε-intro ε-intro
  ε-intro≡ = isProp→PathP (λ i → isLangε≡ i []) _ _

  ε*-intro≡ : PathP (λ i → ε*≡ {ℓ = ℓ} i []) εPath.ε*-intro ε*-intro
  ε*-intro≡ {ℓ = ℓ} = isProp→PathP (λ i → isLangε*≡ {ℓ = ℓ} i []) _ _
  ...
  opaque
    unfolding εPath.ε*-elim ε*-elim
    ε*-elim≡ : PathP (λ i → ε⊢ A → ε*≡ {ℓ = ℓ} i ⊢ A) εPath.ε*-elim ε*-elim
    ε*-elim≡ {A = A} = funExt λ a → funExt λ w →
      funExtDep (λ where
        {lift εP} {lift Eq.refl} eps →
            subst A (sym εP) a
                ≡⟨ cong (λ z → subst A z a) (isSetString _ _ _ _) ⟩
            subst A refl a
                ≡⟨ substRefl {B = A} a ⟩

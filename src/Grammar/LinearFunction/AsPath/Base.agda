...
opaque
  ...
  ⟜-intro : A ⊗ B ⊢ C → B ⊢ C ⟜ A
  ⟜-intro e _ a _ b = e _ ((_ , refl) , (b , a))

  ⟜-app : A ⊗ (B ⟜ A) ⊢ B
  ⟜-app {B = B} _ (s , a , a→b) = subst B (sym (w≡l++r s)) (a→b _ a)
...
opaque
  unfolding _⊸_ _⊗_
  ⊸-intro : A ⊗ B ⊢  C → A ⊢ B ⊸ C
  ⊸-intro e _ a w' b = e _ ((_ , refl) , a , b)

  ⊸-app : (A ⊸ B) ⊗ A ⊢ B
  ⊸-app {B = B} _ (s , a→b , a) = subst B (sym (w≡l++r s)) (a→b _ a)
...
Element→Term {A = A} {B = B} f = ⊸-app ∘g ε-elim f ,⊗ id ∘g ⊗-unit-l⁻

...
module String.Base (Alphabet : Type ℓ-zero) (@0 isSetAlphabet : isSet Alphabet) where
...
isSetString = isOfHLevelList 0 isSetAlphabet
...
SplittingEq w = Σ (Σ String (λ _ → String)) (λ (w₁ , w₂) → w Eq.≡ w₁ ++ w₂)
-- Σ[ (w₁ , w₂) ∈ String × String ] (w Eq.≡ w₁ ++ w₂)
...
module _ (c : Alphabet) where
  @0 splitChar : (w : String) → Splitting (c ∷ w)
  splitChar w = ([ c ] , w) , refl

@0 splitting++ : ∀ w1 w2 → Splitting (w1 ++ w2)
...
-- --     (isPropΣ⊎Split++ isSetAlphabet (s .fst .fst) (s .fst .snd) (s' .fst .fst) (s' .fst .snd))
-- --     (λ x y →
-- --       Sum.rec
-- --         (λ the-split →
-- --           (y .fst .snd)

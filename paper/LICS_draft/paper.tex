...
\begin{definition}
  A model is a locally Cartesian category with two hierarchies of
...
expression relies on additional axioms to the type theory, presented in \cref{fig:brzozowskideriv}.
...
ambiguous \steven{This is an unambiguous grammar, right?} if for every string $w$, the type $G(w \tol A)$ of
...
Classically, formal language theory is closely related to automata theory. In the styles of Chomsky and
Sch\"utzenberger, there is a hierarchy of language classes and the types of automata that recognize them. Inside of the
type theory of \cref{sec:typetheory}, we can characterize these language classes; moreover, we demonstrate equivalence
of each class to the associated automaton as an isomorphism within our logic.
...
We reflect on this expressive power to motivate why we put heavy syntactic restrictions on our presentation of certain
classes of automata. For instance, when defining a pushdown automata, we could use the intuitionistic context to
represent the stack, but we ought to be very careful about how this set is accessed and added to. Careless definitions
may accidentally endow us with too much expressive power, launching us further up in the Chomsky hierarchy. To begin,
let us focus only on language classes, and their associated automata, whose parses are definable without any mention to
the inuitionistic context. When viewed as a machine, this refers to the class of automata definable without access to
a stack/tape of memory --- i.e.\ finite state machines.

\subsection{Finite State Machines}
A \emph{nondeterministic finite automaton} (NFA) is a finite state machine where transitions are labeled with characters from a fixed alphabet $\Sigma$. These are often represented formally as a 5-tuple $(Q, \Sigma, \delta, q_{0}, F)$,

\begin{itemize}
  \item $Q$ a finite set of states
  \item $\Sigma$ a fixed, finite alphabet
  \item $\delta : Q \times \Sigma \to \mathcal{P}(Q)$ a labelled transition function
  \item $q_{0}$ a start state
  \item $F$ a set of accepting states
\end{itemize}

Intuitively, this can be thought of like a directed graph with nodes in $Q$ with an edge $q \overset{c}{\to} q'$ whenever $q' \in \delta(q, c)$. Note that transitions in an NFA may be labelled with the empty string $\epsilon$ --- such transitions are referred to as \emph{$\epsilon$-transitions}.

As a gramamar in our type theory, an NFA may be encoded as a
mutual\footnote{For convenience we refer to a mutual fixed point here, even though we
  only take single fixed points as primitive. As in Beki\'c's theorem, we may decompose our mutual fixed points into a series of single fixed points. So this operation is admissible.}
fixed point in the following manner:

An NFA has finitely many states, so we may number them $Q = \{q_{1}, \dots, q_{N}\}$. For each $q_{j}$ we introduce
a mutually recursive term $Y_{j}$ to represent the states of the NFA within this grammar presentation. We represent the transition function $\delta$ in the right hand side of each $Y_{j}$ definition. For character $c \in \Sigma \cup \{\epsilon\}$ and state $q_{j} \in Q$, we add a summand --- as a coproduct\footnote{There is an admissible notion of coproduct making use of the primitive dependent pairing --- i.e. $A \oplus B := \LinSigTy b \Bool {\textbf{if}~b~A~B}$. We will utilize this without much comment to represent the usual notion of alternation of grammars.} --- to the defintion of $Y_{j}$ for every state in the image of the transition function,

\[
Y_{j} = \bigoplus_{c \in \Sigma \cup \{\epsilon\}} \bigoplus_{q_{i} \in \delta(q_{j}, c)} c \otimes Y_{i}
\]

For convenience, refer to syntactic term on the right hand side of $Y_{j}$ as $\hat{Y}_{j}$. Further, we rename the start state to be represented by the term $Y_{s}$. Additionally, and without loss of generality, we assume that there is a single accepting state $Y_{e}$ in our encoding. This assupmtion is justified through the use of $\epsilon$-transitions. For each $q_{j} \in F$, we add an $\epsilon$-transition from $Y_{j}$ to $Y_{e}$.

The above translation totals to give us a syntactic form for NFAs as grammars,

\[
  NFA ::= \mu \begin{pmatrix}
        Y_{s} = \langle \hat{Y} \rangle \\
        Y_{j} = \langle \hat{Y} \rangle, j \not \in \{s, e\} \\
        Y_{e} = I
        \end{pmatrix}.Y_{s}
\]
where
\[
  \hat{Y} ::= c \otimes Y ~|~ Y ~|~ \hat{Y} \oplus \hat{Y}
\]

and each $Y \in \{ Y_{s}, Y_{e}, Y_{1}, \dots, Y_{N} \}$. Intuitively, we read each $Y_{j}$ as the label for a state, and the
definitions of form $\hat{Y}$ describe the transitions between states --- $c \otimes Y$ denotes a labeled transition, and $Y$
denotes an $\epsilon$-transition. Note that here each $c$ represents a grammar that is inhabited with only a trivial parse of the single character $c$; likwise, and $\epsilon$-transtion corresponds to a transition with label $I$ and $I \otimes Y = Y$ because $I$ is the unit for the operation $\otimes$. A
disjunction of transitions in an NFA grammar corresponds to multiple outgoing edges of a given state in an automaton.

Above we give a translation from the tuple presentation of an NFA to a fixed point grammar in our type theory. We don't provide any argument that this translation is done correctly; however, we hope that the construction is straightforward and lightweight enough to demonstrate our formal system as a natural and effective syntax for reasoning about both grammars and automata.
...
First, let us describe a version of Thompson's construction \cite{?} where we construct an NFA that recognizes a given regular expression. Moreover, we will show that this NFA is strongly equivalent to the original grammar.

\begin{theorem}[Thompson]
  \label{thm:thompson}
  For $g$ a regular grammar $g$, there is an NFA $N$ that recognizes the same language as $g$. Further, $N$ and $g$ are isomorphic.
\end{theorem}
...
We can now inductively define some NFA combinators and show that they preserve grammar equivalence. First begin with
the disjunction of two NFAs. For regular grammars $g$ and $g'$, assume that $g \cong NFA(g)$ and $g' \cong NFA(g')$, and we will define an NFA that
recognizes $g \oplus g'$.

Let the states in $NFA(g)$ be represented by $X$'s and the states in $NFA(g')$ be represented
by $Z$'s. Further let $\hat{X}_{k}$ refer to the syntactic term on the right hand side of the definition of $X_{k}$
inside of $NFA(g)$. Likewise define each $\hat{Z}_{k}$.
...
\[
  ...
  \end{pmatrix}.Y_{s}
\]
where the substitution $\{X_{s}'/X_{s}, X_{e}'/X_{e}, X_{1}'/X_{1}, \dots \}$ is given by $\delta_{X}$ --- and $\delta_{Z}$ constructed similarly.

In the construction of $NFA(g \oplus g')$ we replicate each defintion $X_{j}$ in $NFA(g)$ with an analogous definition
$X_{j}'$ inside of $NFA(g \oplus g')$, except we replace $X_{k}$ with $X_{k}'$ in each production. Lastly, $X_{e}'$ has
an $\epsilon$-transition to the accepting state $Y_{e}$. Likewise, we build analogous defintions for the states
$Z_{j}$ in $NFA(g')$, then we flatten all of these defintions into one mutally recursive list. This has the effect
of disjunctively tying together $NFA(g)$ and $NFA(g')$ into one automaton.

To show the strong equivalence of $NFA(g \oplus g')$ and $g \oplus g'$, we will demonstrate an isomorphism between them
with two proof terms in our logic that mutually invert each other. To define a map from $NFA(g \oplus g')$ to $g \oplus g'$
we will use a multi-fold over all the mutually recursive defintions of $NFA(g \oplus g')$. Note, even though multiple
fixed points are not taken as primitive, using Beki\'c's theorem we can define an admissible axiom schema to define multiple fixed points types. Below we give rules to form $\mathsf{cons}$ and $\mathsf{mfold}$ terms to serve as the introduction and elimination rules, respectively, for multi-term fixed points. First, the introduction rule,
...
\]
where $\sigma$ is the substitution that unrolls the mutually recursive definition one level by replacing each $X_{k}$ with the fixed point $\mu(X_{j} = A_{j}, \forall j).X_{k}$,
...
\[
  \inferrule{\Gamma ; \Delta \vdash e : \mu(X_{1} = A_{1}, \dots, X_{n} = A_{n}).X_{k} \\
...
where the substitution $\gamma$ represents $\{B_{1}/X_{1},\dots,B_{n}/X_{n} \}$.

When defining a derivation from $NFA(g \oplus g')$ to $g \oplus g'$, we
wish to simultaneously fold over the mutually recursive definitions comprising $NFA(g \oplus g')$. We may think of this
as an eliminator that replaces each state inside of $NFA(g \oplus g')$ with a tail-recursive call that will ultimately
return a term of type $g \oplus g'$. If folding over a term $e$, refer to the output of this multifold as $s_{\oplus}(e)$. The structure of such a derivation instatiates the above elimination rule as follow,
...
\[
  ...
  }{\Delta \vdash s_{\oplus}(e) := \mathsf{mfold}(y_{s}.p_{s},\dots,y_{e}.p_{e})(e) : g \oplus g'}
\]

For each of the variables above, we simplify the type and provide the actual proof term. For instance,
$\simulsubst {(X_{s}' \oplus Z'_{s})} \gamma = g \oplus g'$, so the derivation for $p_{s}$ is just the identity.
The rest of the terms are given below,
\begin{itemize}
  \item $y_{s}$ --- $\simulsubst {(X_{s}' \oplus Z'_{s})} \gamma = g \oplus g'$ and $p_{s} = y_{s} : g \oplus g'$
  \item $x_{s}'$ --- $\simulsubst {(\simulsubst {\hat{X}_{s}} {\delta_{X}})}{\gamma} = \mu(X_{i} = \hat{X}_{i}, \forall i).X_{s} = NFA(g)$ and $p_{x,s} = \phi^{-1}(x_{s}') : g$
  \item $x_{j}', j \not \in \{s, e\}$ --- $\simulsubst {(\simulsubst {\hat{X}_{j}} {\delta_{X}})}{\gamma} = \mu(X_{i} = \hat{X}_{i}, \forall i).X_{j}$ and $p_{x, j} = x_{j}' : \mu(X_{i} = \hat{X}_{i}, \forall i). X_{j}$
  \item $x_{e}'$ --- $\simulsubst {Y_{e}}{\gamma} = I$ and $p_{x,e} = x_{e}' : \mu(X_{i} = \hat{X}_{i}, \forall i).X_{e} = I$
  \item $z_{s}'$ --- $\simulsubst {(\simulsubst {\hat{Z}_{s}} {\delta_{Z}})}{\gamma} = \mu(Z_{i} = \hat{Z}_{i}, \forall i).Z_{s} = NFA(g')$ and $p_{z,s} = \psi^{-1}(z_{s}') : g'$
  \item $z_{j}', j \not \in \{s, e\}$ --- $\simulsubst {(\simulsubst {\hat{Z}_{j}} {\delta_{Z}})}{\gamma} = \mu(Z_{i} = \hat{Z}_{i}, \forall i).Z_{j}$ and $p_{z, j} = z_{j}' : \mu(Z_{i} = \hat{Z}_{i}, \forall i).Z_{j}$
  \item $z_{e}'$ ---  $\simulsubst {Y_{e}}{\gamma} = I$ and $p_{z,e} = \mathsf{cons}_{Z_{e}}(z_{e}') : \mu(Z_{i} = \hat{Z}_{i}, \forall i). Z_{e} = I$
  \item $y_{e}$ --- $\simulsubst {I} {\gamma} = I$ and $p_{e} = y_{e} : I$
\end{itemize}

\steven{Question raised by Pranav: why doesn't the $X_{s}'$ being eliminated to $g$ screw up what the other $X_{j}'$s get eliminated too. Need to add an argument that justifies why the $X_{j}$s are allowed to be eliminated via the identity. A priori they could be interfered with by taking in an $X_{s}'$ that needs to be eliminated.

I think the crux of this arugment hinges on a syntactic restriction where there are no backedges to $X_{s}$ in the NFA.\ This is true for the $\otimes$, $\oplus$ cases, unclear in the Kleene star case. Even if there were one, we might be able to get away with a case distinction entailment instead of the one written.}

where $\phi$ and $\psi$ are the witnesses to the isomorphisms $g \cong NFA(g)$ and $g' \cong NFA(g')$, respectively.
This finshes the construction of a parse transformer $s_{\oplus}$ from $NFA(g \oplus g')$ to $ g \oplus g'$.
The map $r_{\oplus}$ in the other direction is given below in a couple stages. First, we show how to turn a parse of $NFA(g)$ into a parse of $NFA(g \oplus g')$.

\[
  \inferrule
  {
    \Gamma ; \Delta \vdash u : NFA(g) = \mu(X_{j} = \hat{X}_{j}, \forall j).X_{s} \\
    \Gamma ; x_{s} : \simulsubst {X_{s}}{\gamma} = X_{s}' \vdash \mathsf{cons}(\textbf{in}_{X_{s}'}(x_{s})) : \mu(\dots).Y_{s} = NFA(g \oplus g')  \\
    \Gamma ; x_{j} : \simulsubst {X_{j}}{\gamma} = \mu(X_{j} = \hat{X}_{j}, \forall j).X_{j}' \vdash x_{j} : \mu(X_{j} = \hat{X}_{j}, \forall j).X_{j}', j \not \in \{s, e\} \\
    \Gamma ; x_{e} : \simulsubst {X_{e}}{\gamma} = I \vdash \mathsf{cons}(x_{e}) : I = \mu(\dots).Y_{e} = \mu(\dots).X_{e}'
  }
  {\Gamma ; \Delta \vdash \mathsf{mfold}(x_{s}.\mathsf{cons}(\textbf{in}_{X_{s}'}(x_{s}), x_{1}.x_{1}, \dots, x_{e}.x_{e}))(u) : NFA(g \oplus g')}
\]
where the dots in the above derivation take on the mutually recursive defintions present in $NFA(g \oplus g')$. \steven{Need a better way to display these defs without blowing up the $\mu$ terms} Denote the above embedding of $NFA(g)$ into $NFA(g \oplus g')$ via $\alpha(u) := \mathsf{mfold}(u) : NFA(g \oplus g')$. Similarly define an embedding $\beta$ for an embedding of $NFA(g')$. We can then define the derivation from $g \oplus g'$ to $NFA(g \oplus g')$ by composing the isomorphisms $g \overset{\phi}{\cong} NFA(g)$ and $g \overset{\psi}{\cong} NFA(g')$ with maps $\alpha$ and $\beta$, respectively.

\[
  \inferrule
  {
    p : g \oplus g' \vdash p : g \oplus g' \\
    p_{1} : g \vdash \alpha(\phi(p_{1})) : NFA(g \oplus g') \\
    p_{2} : g' \vdash \beta(\psi(p_{2})) : NFA(g \oplus g')}
  {
    p : g \oplus g' \vdash r_{\oplus}(p) := \textbf{case}~p \{
    \textbf{in}_{g} p_{1} \mapsto \alpha(\phi(p_{1})), \\
    \textbf{in}_{g'} p_{2} \mapsto \beta(\psi(p_{2})) \} : NFA(g \oplus g')}
\]

We include the proof that maps $s_{\oplus}$ and $r_{\oplus}$ form an isomorphism in \cref{sec:appendixnfa}. This concludes the disjunctive combination of two NFAs.
Because they follow a very similar style of argumentation, we also include the cases for sequencing and
Kleene star in \cref{sec:appendixnfa}. Totaling over all of these cases, this completes the proof.
...
% From here we need to also show that derivatives of regular languages are regular (look at Brzozowski's proof) and that regular languages have finitely many derivatives (potentially with some size constraint on the derivative?). Then the last iso in the chain above can be used to build a DFA equivalent to $A$ using Brzozowski's algorithm.
...
A \emph{deterministic finite automaton} (DFA) is an NFA whose transitions are deterministic. Meaning at every state $q \in Q$ there is exactly one transition for every character $c \in \Sigma$ --- i.e.\ the size of $\delta(q, c)$ is 1 and $\delta(q, \epsilon)$ is empty. It is clear that all DFAs happen to be NFAs, but it is perhaps surprising that the class of DFAs is as powerful as the class of NFAs when measured by the languages they recognize! \cref{thm:thompson} shows that the class of languages recognized by NFAs is exactly the regular languages. As we will soon see, the class of languages recognizable by DFAs is also the regular languages!

There are a couple famous constructions we might use to show this, such as the powerset construction \cite{?} or Brzozowski's algorithm \cite{?}. Just as with Thompson's construction, we shall internalize a version of the latter as a proof term in our formal calculus. However, note that the construction of a DFA is lossy in some sense. That is, throughout this process we lose the ability to form an isomorphism between parses of a DFA for $g$ and parses of $g$ itself. Rather than express an isomorphism (strong equivalence) as a proof term, we shall construct a weak equivalence. This means we shall derive entailments $g \vdash DFA(g)$ and $DFA(g) \vdash g$ that do not necessarily invert each other. Semantically, this notion of weak equivalence between grammars $g$ and $g'$ means that $g$ and $g'$ both recognize the same language.

Recall that a DFA is given as a 5-tuple $(Q, \Sigma, \delta, q_{0}, F)$ in the same manner as an NFA.\ When encoding a DFA as a mutually recursive grammar, we use nearly the same process as when we encoded an NFA, with one subtle change. When we encoded NFAs, we were able to ensure we had a single accepting state by connecting all of the accepting states $q_{j} \in F$ to a single accepting state $Y_{e}$ with $\epsilon$-transitions. However, $\epsilon$-transitions are not permitted in a DFA, as they introduce a source of nondeterminism. Therefore we need a scheme to encode which states are accepting. The changes we need to make is pretty lightweight. First, we add the grammar $I$ as a summand to distinguish which states are accepting. Second, each state must have a transition for every character in the alphabet.

\[
  DFA ::= \mu \begin{pmatrix}
        Y_{s} = \langle \hat{Y} \rangle \\
        Y_{j} = \langle \hat{Y} \rangle, j \not \in \{s, e\} \\
        \end{pmatrix}.Y_{s}
\]
where
\[
\hat{Y} ::= \hat{Y_{0}} ~|~ I \oplus \hat{Y_{0}}
\]
\[
  \hat{Y_{0}} ::= \bigoplus_{c : \Sigma} c \otimes Y, Y \in \{Y_{s}, Y_{1}, \dots, Y_{N}\}
\]

A state $Y_{j}$ in a DFA is accepting if and only if $\hat{Y}_{j}$ contains $I$ as a summand.

In the definition of a DFA, we make use of a sum ranging over all characters in the alphabet. Viewed as a machine, this is intened to have the effect of being able to run any arbitrary string input through the automaton to reach some well-defined, but not necessarily accepting, state. In our type theory, this would amount to the following entailment being derivable,

\steven{This might be the wrong entailment, and I may instead want a propositional truncation. This distinction feels related to the hang ups in the second half of the DFA proof}

\[
p : \top \vdash \mathsf{run}(p) : \mu \begin{pmatrix}
        Y_{s} = \langle \hat{Y} \rangle \\
        Y_{j} = \langle \hat{Y} \rangle, j \not \in \{s, e\} \\
        \end{pmatrix}.\bigoplus_{k} Y_{k}
\]

for some term $\mathsf{run}(p)$ that might depend on the structure of the DFA in question. That is, any input should be runnable through the DFA to return \emph{some} state. This entailment holds in the intended model, but a priori this does not hold in the type theory. Noting this point motivates taking $\top \cong (\bigoplus_{c} c)^{*}$ as an axiom. This syntactically reflects the semantic notion that parses correspond to finite strings over a fixed alphabet. Such an axiom is especially justified when crafting syntax to reason about parses of finite input strings; further, we conjecture that without this axiom we may find models of the type theory that correspond to parses of infinite sequences of characters. In this paper, we shall focus on the expressivity of the syntax to reason about parses of strings, and thus we assume $\top \cong (\bigoplus_{c} c)^{*}$.

Additionally, we assume that the derivation from $c \otimes (c \lto A)$ to $(c \otimes \top) \amp A$ is an isomorphism. Specifically, the term $\phi$ defined below is assumed to have an inverse,

\[
p : c \otimes (c \lto A) \vdash \phi(p) = \letin {p_{1} \otimes p_{2}} p {(p_{1} \otimes \top, p_{1}^{\lto}p_{2})} : (c \otimes \top) \amp A
\]

Axiomatically adding an inverse for $\phi$ doesn't greatly affect our semantics. In fact, it describes a property that is simply true in the intended string model: ``if a parse of character $c$ and then a parse of any word completes to a parse of $A$, then that word was actually in fact of type $c \lto A$''. This axiom, the one desribed in the paragraph above, and the ones given in \cref{fig:brzozowskideriv} are not meant to anything particularly deep. They are a means of reflecting the intended semantic properties in our syntax.

Utilizing the reflected properties we can derive the following isomorphism,

\begin{align*}
  A
  & \cong \top \amp A \\
  & \cong (\bigoplus_{c} c)^{*} \amp A \\
  & \cong (I \oplus \bigoplus_{c} (c \otimes \top)) \amp A \\
  & \cong (I \amp A) \oplus \bigoplus_{c} ((c \otimes \top) \amp A) \\
  & \cong (I \amp A) \oplus \bigoplus_{c} (c \otimes (c \lto A))
\end{align*}

Which effectively states that parses of a grammar $A$ are either empty or begin with some character from our fixed alphabet, a fact used in our proof of \cref{thm:dfa}.

\begin{theorem}
  \label{thm:dfa}
  For every regular grammar $g$, there is a DFA $D$ that is weakly equivalent to $g$.
\end{theorem}

\begin{proof}
  First, note that regular grammars have finitely many derivatives, up to weak equivalence \cite{?}. That is, we can isolate a finite set of words $w_{1},\dots,w_{n}$ such that $w \lto g$ is weakly equivalent to $w_{j} \to g$ for some $g$. These $w_{j}$ may then serve as representatives for their weak equivalences classes.

  The high-level idea underlying Brzozowski's DFA construction algorithm is that the states of the DFA are the derivative equivalence classes, and the transitions are formed by taking derivatives. If the derivative of $w \lto g$ with respect to a character $c$ is weakly equivalent to $w' \lto g$ then there is a transition from the state representing $w \lto g$ to the state representing $w' \lto g$. These pieces of data form the transition table form our DFA.\ Lastly, we make the state representing $w \lto g$ accepting if the type $I \amp (w \lto g)$ is inhabited. If it is inhabited the state for $w \lto g$ will have an $I$ as a summand, otherwise it will not. To represent this acceptance formally in our construction, use the function symbol $\mathsf{ac}(w) : 0 \oplus I$ to be $I$ if $w \lto g$ is inhabited and 0 otherwise. Similarly, $\mathsf{ac}(I)$ will represent the acceptance of the start state.

  We may precompute all this info and then construct the DFA by introducing states labeled $Y_{j}$ for each representative $w \in \{w_{1}, \dots, w_{n}\}$. Letting $j$ range in $\{s, 1, \dots, n\}$ and taking $w_{s} = I$ we can describe all states of the DFA with a similar structure.

  \[
  DFA(g) =
  \mu \begin{pmatrix}
        Y_{j} = \mathsf{ac}(w_{j}) \oplus \bigoplus_{c} c \otimes Y_{n_{j,c}} \\
  \end{pmatrix}.Y_{s}
\]

Note that by construction the target state of each transition, $Y_{n_{j,c}}$, is such that $c \lto (w_{j} \lto g)$ is weakly equivalent to $w_{n_{j, c}} \lto g$.

To turn a term of type $DFA(g)$ into a term of type $g$, we simply fold over the definitions of $DFA(g)$. Again, the structure of such a derivation,

\[
  \inferrule
  {
    \Gamma ; \Delta \vdash e : DFA(g) = \mu(Y_{j} = \hat{Y}_{j}).Y_{s} \\
    \Gamma ; x_{s} : \simulsubst {\mathsf{ac}(w_{j}) \oplus \bigoplus_{c} c \otimes Y_{n_{s,c}}} {\gamma} = \mathsf{ac}(w_{j}) \oplus \bigoplus_{c} c \otimes B_{n_{s,c}} \vdash e_{s} : B_{s} \\
    \Gamma ;x_{j} : \simulsubst {\mathsf{ac}(w_{j}) \oplus \bigoplus_{c} c \otimes Y_{n_{j,c}}}{\gamma} = \mathsf{ac}(w_{j}) \oplus \bigoplus_{c} c \otimes B_{n_{j,c}} \vdash e_{j} : B_{j}
  }
  {\Gamma ; \Delta \vdash \mathsf{mfold}(x_{s}.e_{s}, x_{1}.e_{1}, \dots, x_{n}.e_{n})(e) : B_{s}}
\]

When instantiating the above rule, we make each $B_{k} = w_{k} \lto g$. In this manner, $B_{s} = I \lto g = g$ which is our desired output type. Let us examine one of the entailments to see how this is achievable. From a term $x_{j} : \mathsf{ac}(w_{j}) \oplus \bigoplus_{c} c \otimes (w_{n_{j,c}} \lto g)$, we need to produce some $e_{j} : w_{j} \lto g$. Which we may do in the following manner,

\begin{align*}
    \begin{aligned}
  e_{j} =
    \phi(\textbf{case}~x_{j}~\{&  \textbf{in}_{\mathsf{ac}} u \mapsto \mathbf{in}_{I \amp (w_{j} \to g)} u, \\
  & \textbf{in}_{c \otimes (w_{n_{j,c}} \lto g)} (v_{c} \otimes d_{j,c}) \mapsto \mathbf{in}_{c \otimes (w_{j} \lto g)}(v_{c} \otimes \alpha(d_{j,c}))
  \})
    \end{aligned}
\end{align*}

Where $\phi$ witnesses the isomorphism $g \cong (I \amp g) \oplus \bigoplus_{c} (c \otimes (c \lto g))$, and $\alpha$ is a function that acts a lookup table for replacing $d_{j,c} : w_{n_{j,c}} \lto g$ with an appropriate member of its weak equivalence class. In particular, when deriving $w_{j} \lto g$ we want $\alpha(d_{j,c}) : (w_{j} \otimes c) \lto g$. Finding such an element is indeed possible \emph{by construction of our DFA}. Recall that we added a transition between $Y_{j}$ and $Y_{n_{j,c}}$ exactly when $c \lto (w_{j} \lto g) = (w_{j} \otimes c) \lto g$ was weakly equivalent to $w_{n_{j,c}} \lto g$. That is, the multifold constructed above is well-defined, and will ultimately return a term of type $B_{s} = I \lto g = g$ so we do, in fact, have a derivation from $DFA(g)$ to $g$.

Now we wish to build a derivation from $g$ to $DFA(g)$. TODO proof.
\steven{
  I think there is a way to do this via extracting some string related to a parse. i.e. $p : g \vdash M : (\LinSigTy {n} {\mathbb{N}} {\text{Char}^{n}} \amp g)$. However, Max said that this style of argument --- or perhaps just pattern matching over the sigma type --- requires full bunched contexts. I don't totally see why.

  The hope is to then morally run the DFA on the string, and you can show that it must be accepting by some of the properties used to construct the DFA.

  An alternative to the above approach is to reflect the fact that grammars are weakly equivalent if they recognize all the same strings. Something like,

  \[
    \inferrule
    {\Gamma \vdash M : \Pi_{w} G(w \lto A) \to G(w \to B)}
    {\Gamma \vdash Sem(M) : G(A \lto B)}
  \]

  You then instantiate this rule for $A = g$, $B = DFA(g)$. This gives you access to the same kind of string I am trying to access above and you make the same kind of argument. I'm too tired to write the stub of that argument now, and I'm a little shaky on it, but that will be a goal for next week.
}
\end{proof}
...
dependency of linear types on non-linear types. In this section, we shall explore how we can perform these encodings for pushdown automata and Turing machines. In this paper, we don't provide any proofs of their equivalence to context-free or recursively-enumerable languages; however, to motivate this type theory as a natural and general syntax for reasoning about both languages and machines, we do show how to internalize these notions of automata.

\subsubsection{Pushdown Automata}
Let us recall that a nondeterministic \emph{pushdown automaton} (PDA) is a type of automaton that utilizes a stack of memory. Similarly to NFAs, they are often represented as 7-tuples $(Q, \Sigma, \Gamma, \delta, q_{0}, Z, F)$. Here $Q, \Sigma, q_{0},$ and $F$ all have the exact same meanings as with NFAs. Additionally PDAs now have,

\begin{itemize}
\item $\Gamma$ a fixed, finite stack alphabet. We interpret ``the stack'' as words over $\Gamma$
\item $Z$ an initial stack symbol
\item $\delta : Q \times (\Sigma \cup \{\epsilon\}) \times \Gamma \to Q \times \Gamma^{*}$ a transition function that consumes a state, input character, and stack character then returns a state and new stack
\end{itemize}

Just as with NFAs, we encode PDAs via a particular mutual fixed point type, except now we let these types make use of non-linear types. In particular, let $S$ be a finite
...
Like with NFAs, we introduce a label $Y_{j}$ for each $q_{j} \in Q$ but now the right hand side of each of their definition is a dependent type that takes into account the stack. We again say there is a single accepting state. The initial stacks $Z$ corresponds to the empty list $\mathsf{nil}$. Additionally, without loss of generality we say that every transition consumes the head of the stack --- as if a transition did not consume anything, you could pop and replace the head with the same element.

Consider a transition in the PDA between states $Y_{i}$ and $Y_{j}$ with current stack $\omega_{0} \in \Gamma^{*}$ that consumes stack character $a$, consumes an input character $c$, and then pushes $\omega \in \Gamma^{*}$ onto the stack. We translate this into the definition of $Y_{i}$. $\omega_{0}$ will correspond to some current list $L$, and the production on the right hand side of $Y_{i}$ will take the form of a (partial) case distinction on the head of $L$. The case corresponding to $a$ will be a dependent type that encodes the pushing of $\omega$ onto the stack as appending some list $L_{0}$ to the tail of $L$. That is, the definition of $Y_{i}$ looks like,

\[
Y_{i} = \LinSigTy {L} {\mathrm{List}\,S} {\textbf{case}~L~\{(a, tl) \mapsto \LinSigTy {\mathsf{append}(L_{0}, tl)} {\mathrm{List}\,S} {c \otimes Y_{j}}, \dots\}}
\]

In total,

\steven{idk why I'm giving these as meta-level grammars. It doesn't seem like an illuminating way to share the form for PDAs, but I can't think of anything cleaner. This can definitely be put more cleanly}

\[
  PDA ::= \mu
  \begin{pmatrix}
    Y_{s} = \langle \hat{P}_{s} \rangle \\
    Y_{j} = \langle \hat{P} \rangle \\
    Y_{e} = \LinSigTy {L} {\textrm{List}\,S} {I}
  \end{pmatrix}.Y_{s}
\]

\[
\hat{P} ::= \LinSigTy{L}{\textrm{List}\,S}{\textbf{case}~L~\{ \langle cases \rangle \}}
\]
\[
\hat{P}_{s} ::= \LinSigTy{\mathsf{nil}}{\textrm{List}\,S}{\textbf{case}~L~\{ \langle cases \rangle \}}
\]
\[
cases ::= case ~|~ cases, case
\]
\[
case ::= (z, tl) \mapsto T_{\oplus}
\]
\[
  T_{\oplus} ::= T ~|~ T \oplus T_{\oplus}
\]
\[
T ::= \LinSigTy{L'}{\textrm{List}\,S}{c \otimes Y}
\]

\subsubsection{Turing Machines}
We can encode a Turing machine nearly identically to how we encoded PDAs. Instead of having our encoding depend on a non-linear type of lists over $S$, we have it depend on a pair of type $\mathrm{List}\,S \times \mathrm{List}\,S$.
As mentioned earlier \steven{Probably copy that earlier bit from the beginning of the section here and reword}, we can simulate the infinite tape of a Turing machine with two stacks.

\steven{I'm satisfied giving this high-level intuition and not dumping a syntactic mess for the audience, but as always its useful to have explicit examples because the claim is that the syntax is natural and easy to use.}
...
\section{Equivalence Betweeen Regular Grammars and Finite Automata}
\label{sec:appendixnfa}

Below we finish the proof of \cref{thm:thompson}.
\begin{proof}

  First, we finish the provide the proof that the maps $s_{\oplus}$ and $r_{\oplus}$ provided in the disjunctive case do indeed mutually invert each other. For brevity in this proof, we will omit the list of mutually recursive bindings when appropriate. That is, write $\mu X_{s}'$ instead of $\mu(Y_{s},X_{s}',X_{j}',\dots). X_{s}'$.

  Given $e : NFA(g \oplus g')$,

\begin{align*}
  r_{\oplus}(s_{\oplus}(e))
  & =_{\mu \eta} r_{\oplus}(s_{\oplus}(\mathsf{cons}(e'))), \text{~where }e':\mu X_{s}' \oplus \mu Z_{s}' \\
  &
    \begin{aligned}
      =_{\oplus \eta} r_{\oplus}(s_{\oplus}(\mathsf{cons}(\textbf{case}~e'~\{
    & \textbf{in}_{\mu X_{s}'} u \mapsto \textbf{in}_{\mu X_{s}'} u, \\
    & \textbf{in}_{\mu Z_{s}'} v \mapsto \textbf{in}_{\mu Z_{s}'} v
    \})))
    \end{aligned} \\
  &
    \begin{aligned}
    =_{\oplus \eta}
    \textbf{case}~e'~\{
    & \textbf{in}_{\mu X_{s}'} u \mapsto r_{\oplus}(s_{\oplus}(\mathsf{cons}(\textbf{in}_{\mu X_{s}'} u))), \\
    & \textbf{in}_{\mu Z_{s}'} v \mapsto r_{\oplus}(s_{\oplus}(\mathsf{cons}(\textbf{in}_{\mu Z_{s}'} v)))
    \}))
    \end{aligned} \\
  &
    \begin{aligned}
    =
    \textbf{case}~e'~\{
    & \textbf{in}_{\mu X_{s}'} u \mapsto r_{\oplus}(\mathsf{mfold}(\mathsf{cons}(\textbf{in}_{\mu X_{s}'} u))), \\
    & \textbf{in}_{\mu Z_{s}'} v \mapsto r_{\oplus}(\mathsf{mfold}(\mathsf{cons}(\textbf{in}_{\mu Z_{s}'} v)))
    \}
    \end{aligned} \\
  &
    \begin{aligned}
    =_{\mu \beta}
    \textbf{case}~e'~\{
    & \textbf{in}_{\mu X_{s}'} u \mapsto r_{\oplus}(\textbf{in}_{g} \phi^{-1}(\mathsf{cons}_{\mu(X_{i}=\hat{X}_{i}).X_{s}}u')), \\
    & \textbf{in}_{\mu Z_{s}'} v \mapsto r_{\oplus}(\textbf{in}_{g'} \psi^{-1}(\mathsf{cons}_{\mu(Z_{i}=\hat{Z}_{i}).Z_{s}}v'))
      \} \\
      & \text{~for }u' : \mu X_{s}', v' : \mu Z_{s}'
    \end{aligned} \\
  &
    \begin{aligned}
    =_{\oplus \beta}
    \textbf{case}~e'~\{
    & \textbf{in}_{\mu X_{s}'} u \mapsto \alpha \phi \phi^{-1}(\mathsf{cons}(u')), \\
    & \textbf{in}_{\mu Z_{s}'} v \mapsto \beta \psi \psi^{-1}(\mathsf{cons}(v'))
      \}
    \end{aligned} \\
  &
    \begin{aligned}
    =
    \textbf{case}~e'~\{
    & \textbf{in}_{\mu X_{s}'} u \mapsto \alpha (\mathsf{cons}(u')), \\
    & \textbf{in}_{\mu Z_{s}'} v \mapsto \beta (\mathsf{cons}(v'))
      \}
    \end{aligned} \\
   &
    \begin{aligned}
    =_{\mu \beta}
    \textbf{case}~e'~\{
    & \textbf{in}_{\mu X_{s}'} u \mapsto \mathsf{cons}(\textbf{in}_{\mu X_{s}'} u) , \\
    & \textbf{in}_{\mu Z_{s}'} v \mapsto \mathsf{cons}(\textbf{in}_{\mu Z_{s}'} v)
      \}
    \end{aligned} \\
  & =_{\oplus \eta} \mathsf{cons}(e') \\
  & =_{\mu \beta} e
\end{align*}

Likewise for $p : g \oplus g'$,
...
\begin{align*}
  ...
  &
    =_{\oplus \eta}
      \textbf{case}~p~\{
    \begin{aligned} \\
    & \textbf{in}_{g}p_{1} \mapsto s_{\oplus}(r_{\oplus}(\textbf{in}_{g}p_{1})), \\
    & \textbf{in}_{g'}p_{2} \mapsto s_{\oplus}(r_{\oplus}(\textbf{in}_{g'}p_{2})) \}
    \end{aligned} \\
   & \begin{aligned}
    =_{\oplus \beta}
      \textbf{case}~p~\{
    & \textbf{in}_{g}p_{1} \mapsto s_{\oplus}(\alpha \phi p_{1}), \\
    & \textbf{in}_{g'}p_{2} \mapsto s_{\oplus}(\beta \psi p_{2}) \}
    \end{aligned} \\
   & \begin{aligned}
    =
      \textbf{case}~p~\{
    & \textbf{in}_{g}p_{1} \mapsto \mathsf{mfold}(\alpha \phi p_{1}), \\
    & \textbf{in}_{g'}p_{2} \mapsto \mathsf{mfold}(\beta \psi p_{2}) \}
    \end{aligned} \\
    & \begin{aligned}
    =_{\mu \beta}
      \textbf{case}~p~\{
    & \textbf{in}_{g}p_{1} \mapsto \textbf{in}_{g} (\phi^{-1} \phi p_{1}), \\
    & \textbf{in}_{g'}p_{2} \mapsto \textbf{in}_{g'}(\psi^{-1} \psi p_{2}) \}
    \end{aligned} \\
     & \begin{aligned}
    =
      \textbf{case}~p~\{
    & \textbf{in}_{g}p_{1} \mapsto \textbf{in}_{g} (p_{1}), \\
    & \textbf{in}_{g'}p_{2} \mapsto \textbf{in}_{g'}(p_{2}) \}
    \end{aligned} \\
  &
    =_{\oplus \eta} p
\end{align*}

Thus $s_{\oplus}$ and $r_{\oplus}$ form the isomorphism $NFA(g \oplus g') \cong g \oplus g'$.
...
Just as before, we use a multifold to define the map $s_{\otimes}$ from $NFA(g \otimes g')$ to $g \otimes g'$. Here the table of terms is as follows,

\begin{itemize}
  \item $y_{s}$ --- $\simulsubst {X_{s}'} \gamma = g \otimes g'$ and $p_{s} = y_{s} $
  \item $x_{s}'$ --- $\simulsubst {(\simulsubst {\hat{X}_{s}} {\delta_{X}})}{\gamma} = (\mu (X_{i}=\hat{X}_{i}).X_{s}) \otimes g' = NFA(g) \otimes g'$ and $p_{x,s} = \letin {u \otimes v} {x_{s}'} {\phi^{-1}(u) \otimes v} : g \otimes g'$
  \item $x_{j}', j \not \in \{s, e\}$ --- $\simulsubst {(\simulsubst {\hat{X}_{j}} {\delta_{X}})}{\gamma} = (\mu (X_{i}=\hat{X}_{i}).X_{j}) \otimes g'$ and $p_{x, j} = x_{j}' : (\mu (X_{i}=\hat{X}_{i}).X_{j}) \otimes g'$
  \item $x_{e}'$ --- $\simulsubst {Z_{s}'}{\gamma} = g' = I \otimes g'$ and $p_{x,e} = \letin {mt \otimes p_{g'}} {\mathsf{cons}(mt) \otimes p_{g'}} x_{e}' : (\mu (X_{i}=\hat{X}_{i}).X_{e}) \otimes g'$
  \item $z_{s}'$ --- $\simulsubst {(\simulsubst {\hat{Z}_{s}} {\delta_{Z}})}{\gamma} = \mu (Z_{i} = \hat{Z}_{i}).Z_{s} = NFA(g')$ and $p_{z,s} = \psi^{-1}(z_{s}') : g'$
  \item $z_{j}', j \not \in \{s, e\}$ --- $\simulsubst {(\simulsubst {\hat{Z}_{j}} {\delta_{Z}})}{\gamma} = \mu (Z_{i} = \hat{Z}_{i}).Z_{j}$ and $p_{z, j} = z_{j}' : \mu (Z_{i} = \hat{Z}_{i}).Z_{j}$
  \item $z_{e}'$ ---  $\simulsubst {Y_{e}}{\gamma} = I$ and $p_{z,e} = \mathsf{cons}(z_{e}') : \mu (Z_{i} = \hat{Z}_{i}).Z_{e} = I$
  \item $y_{e}$ --- $\simulsubst {I} {\gamma} = I$ and $p_{e} = y_{e} : I$
\end{itemize}

The map $r_{\otimes}$ in the other direction is given again in two steps. Like the disjunctive case, we give maps $\alpha$ and $\beta$. First, $\beta$,

\[
  \inferrule
  {
    \Gamma; \Delta \vdash e : NFA(g') = \mu(Z_{j} = \hat{Z}_{j}).Z_{s} \\
    \Gamma; z_{s} : \simulsubst {Z_{s}} \gamma = \mu Z_{s}' \vdash z_{s} : \mu Z_{s}' \\
    \Gamma; z_{j} : \simulsubst {Z_{j}} \gamma = \mu Z_{j}' \vdash z_{j} : \mu Z_{j}' \\
    \Gamma; z_{e} : \simulsubst {Z_{e}} \gamma = I \vdash \mathsf{cons}_{Z_{e}'}(\mathsf{cons}_{Y_{e}}(z_{e})) : \mu Z_{e}'
  }
  {\Gamma ; \Delta \vdash \beta(e) := \mathsf{mfold}(z_{s}.z_{s},\dots,z_{e}.\mathsf{cons}_{Z_{e}'}(\mathsf{cons}_{Y_{e}}(z_{e})))(e) : \mu Z_{s}
  }
\]

Note that even the rule $\alpha$ shown below doesn't strictly follow the syntax for an $\mathsf{mfold}$, it can actually be shown to be admissible\footnote{To do so, one could define a new automata-like fixed point where the end state of NFA(g) is replaced with a term of type $\mu Z_{s}$}. This presentation is more compact and to the point,

\[
  \inferrule
  {
    \Gamma; \Delta \vdash e : NFA(g) \otimes \mu Z_{s}' = \mu(X_{j} = \hat{X}_{j}).X_{s} \otimes \mu Z_{s}' \\
    \Gamma; x_{s} : \simulsubst {X_{s}} \gamma = \mu X_{s}' \vdash x_{s} : \mu X_{s}' \\
    \Gamma; x_{j} : \simulsubst {X_{j}} \gamma = \mu X_{j}' \vdash x_{j} : \mu X_{j}' \\
    \Gamma; x_{e} : \simulsubst {X_{e}} \gamma = I \otimes \mu Z_{s}' = \mu Z_{s}' \vdash \mathsf{cons}(x_{e}) : \mu X_{e}'
  }
  {
    \Gamma ; \Delta \vdash \alpha(e) := \mathsf{mfold}(x_{s}.x_{s},x_{1}.x_{1},\dots,x_{e}.\mathsf{cons}(x_{e}))(e) : \mu X_{s}'
  }
\]

We then leverage these $\alpha$ and $\beta$ to build $r_{\otimes}$,

\[
  \inferrule{
    p : g \otimes g' \vdash p : g \otimes g' \\
    p_{1} : g, p_{2} : g' \vdash \mathsf{cons}(\alpha(\phi(p_{1}) \otimes \beta(\psi(p_{2})))) : NFA(g \otimes g')}
  {p : g \otimes g' \vdash \letin {p_{1} \otimes p_{2}} {p} {\mathsf{cons}(\alpha(\phi(p_{1}) \otimes \beta(\psi(p_{2}))))} : NFA(g \otimes g')}
\]

We now show that $s_{\otimes}$ and $r_{\otimes}$ form an isomorphism.

\steven{This proof is a little sketchy. I'm not sure how to best write that the terms with $m$ and $n$ arise via functorial action by the term in the third equality. Moreover, I'm not sure that this third equality term is the right one}

\begin{align*}
r_{\otimes}(s_{\otimes}(e))
  & =_{\mu \eta} r_{\otimes}(s_{\otimes}(\mathsf{cons}(e'))) \\
  & = r_{\otimes}(\mathsf{mfold}(\mathsf{cons}(\mathsf{mfold}(y_{s}.y_{s},\dots,y_{e}.y_{e})(e)))) \\
  &
    \begin{aligned}
    =_{\mu \beta}
    r_{\otimes} (
    & (\letin {u \otimes v} {x_{s}'} {\phi^{-1}(u) \otimes v}) \\
    & (\mathsf{mfold}(y_{s}.y_{s},\dots,y_{e}.y_{e})(e)))
    \end{aligned}
     \\
  & = r_{\otimes}(\phi^{-1}(m) \otimes \psi^{-1}(n)) \\
  & = \mathsf{cons}(\alpha(\phi \phi^{-1} m \otimes \beta(\psi \psi^{-1} n))) \\
  & = \mathsf{cons}(\alpha(m \otimes \beta(n))) \\
  & = \mathsf{cons}(\mathsf{mfold}(x_{s}.x_{s}, \dots, x_{e} . \mathsf{cons}(x_{e}))(m \otimes \beta (n))) \\
  & = \mathsf{cons}(\mathsf{mfold}(y_{s}.y_{s}, \dots, y_{e} . y_{e})(e)) \\
  & =_{\mu \eta} e
\end{align*}

\begin{align*}
  s_{\otimes}(r_{\otimes}(p))
  & =_{\otimes \eta} s_{\otimes}(r_{\otimes}(p_{g} \otimes p_{g'})) \\
  & = s_{\otimes}(\mathsf{cons}(\alpha(\phi p_{g} \otimes \beta(\psi p_{g'}))) \\
  & =_{\mu \beta} \phi^{-1}\phi p_{g} \otimes \psi^{-1} \psi p_{g'} \\
  & = p_{g} \otimes p_{g'} \\
  & =_{\otimes \eta} p
\end{align*}

Thus $s_{\otimes}$ and $r_{\otimes}$ form the isomorphism $NFA(g \otimes g') \cong g \otimes g'$.
...
Again we define $s_{*}$ as a fold over these defintions to get a parse of $g^{*}$,
\begin{itemize}
  ...
  \item $x_{s}'$ --- $\simulsubst {(\simulsubst {\hat{X}_{s}} {\delta_{X}})}{\gamma} = (\mu(X_{i}=\hat{X_{i}}).X_{s}) \otimes g^{*} = NFA(g) \otimes g^{*}$ and $p_{x,s} = \letin {u \otimes p_{*}} {x_{s}'} {\mathsf{cons}_{*}(\phi^{-1}(u), p_{*})} : g^{*} $
  \item $x_{j}', j \not \in \{s, e\}$ --- $\simulsubst {(\simulsubst {\hat{X}_{j}} {\delta_{X}})}{\gamma} = (\mu(X_{i}=\hat{X_{i}}).X_{j}) \otimes g^{*}$ and $p_{x, j} = : (\mu(X_{i}=\hat{X_{i}}).X_{j}) \otimes g^{*}$
  \item $x_{e}'$ --- $\simulsubst {(X_{s}' \oplus Y_{e})}{\gamma} = g^{*} \oplus I$ and $p_{x,e} = \mathsf{cons}_{X_{e}}(()) \otimes \textbf{case}~x_{e}' \{ \textbf{in}_{g^{*}} u \mapsto u, \textbf{in}_{I} v \mapsto \mathsf{cons}_{*I} v \} : (\mu(X_{i}=\hat{X_{i}}).X_{e}) \otimes g^{*}$
  \item $y_{e}$ --- $\simulsubst {I} {\gamma} = I$ and $p_{e} = y_{e} : I$
\end{itemize}

and to derive a parse of $NFA(g^{*})$ from a parse of $g^{*}$ we define $r_{*}$ as,

\[
  \inferrule{p : g^{*} \vdash p : g^{*} \\ \cdot \vdash \mathsf{cons}(\textbf{in}_{{Y_{e}}}(())) : NFA(g^{*}) \\ u : g, v : g^{*} \vdash \mathsf{cons}(\textbf{in}_{{X_{s}'}} (\phi(\mathsf{cons}_{*}(u, v)))) : NFA(g^{*}) }{p : g^{*} \vdash \mathsf{fold}_{*}(\mathsf{cons}(\textbf{in}_{{Y_{e}}}(())), \mathsf{cons}(\textbf{in}_{{X_{s}'}} (\phi(\mathsf{cons}_{*}(u, v)))) : NFA(g^{*}) }
\]

To see that $s_{*}$ and $r_{*}$ for an isomorphism,
\steven{This proof also needs more work. Esp after 4th equality. Putting a pin in both directions of the Kleene star equality proof until I fully understand the issue with the $\otimes$ case. Each of these stems from an ignorance on $\mu$-type $\beta$ and $\eta$ rules}
\begin{align*}
  r_{*}(s_{*}(e))
  & =_{\mu \eta} r_{*}(s_{*}(\mathsf{cons}(e'))), \text{~where }e':\mu X_{s}' \oplus Y_{e} \\
  &
    \begin{aligned}
      =_{\oplus \eta}
      \textbf{case}~e'~\{ & \\
      & \textbf{in}_{X_{s}'} u \mapsto r_{*}(s_{*}(\mathsf{cons}(\textbf{in}_{X_{s}'} u))), \\
      & \textbf{in}_{Y_{e}} v \mapsto r_{*}(s_{*}(\mathsf{cons}(\textbf{in}_{Y_{e}} v))) \\
      \}
    \end{aligned} \\
  &
    \begin{aligned}
      =_{\mu \beta}
      \textbf{case}~e'~\{ & \\
      & \textbf{in}_{X_{s}'} u \mapsto r_{*}(\mathsf{cons}_{*}(\phi^{-1} u, p_{*})), \\
      & \textbf{in}_{Y_{e}} v \mapsto r_{*}(\mathsf{cons}_{*}(()) \\
      \}
    \end{aligned} \\
  &
    \begin{aligned}
      =_{\mu \beta}
      \textbf{case}~e'~\{ & \\
      & \textbf{in}_{X_{s}'} u \mapsto \mathsf{cons}(\textbf{in}_{X_{s}'}(\phi(\mathsf{cons}_{*}(\phi^{-1}u,p_{*})))), \\
      & \textbf{in}_{Y_{e}} v \mapsto \mathsf{cons}(\textbf{in}_{Y_{e}}(()) \\
      \}
    \end{aligned} \\
  & = \dots \\
  & = e
...
\begin{align*}
  s_{*}(r_{*}(p))
  & =_{\mu \eta} s_{*}(r_{*}(\mathsf{cons}(p_{e}, p_{*}))), \text{~where } p_{e} : I, p_{*} : g \otimes g^{*} \\
  & =_{\mu \beta} s_{*}()
  & = p
\end{align*}

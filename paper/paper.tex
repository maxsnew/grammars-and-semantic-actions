...
\begin{definition}
  ...
  By induction we can define an identity substitution $\textrm{id} :
  \textrm{Subst}(\Gamma,\Gamma)$ which maps each variable to itself,
  and composition of substitutions
  \begin{align*}
  ...
  the type theory that are parameterized by a $\Gamma$ support an
  admissible action of substitution:
  \begin{mathpar}
    \inferrule
    {\Gamma \vdash M : X \and \gamma \in \Subst(\Gamma',\Gamma)}
    {\Gamma' \vdash M[\gamma] : X[\gamma]}

    \inferrule
    {\Gamma;\Delta \vdash e : A \and \gamma \in \Subst(\Gamma',\Gamma)}
...
\begin{definition}
  ...
  \Subst(\Delta',\Delta)$, we define the substitution $\Gamma; \Delta
  \vdash e[\delta] : A$ as follows, frequently using the inversion
  principle to split the substitution into constituent components.
  \begin{align*}
    a[e/a] &= e\\
...
Next we define the term semantics for linear terms.

...
\newcommand{\StringSem}{\textbf{String}}
...
automaton that is formally verified to implement an $\LR{1}$ grammar
...
$\w$. The term $a : A$ represents a \emph{parse tree} of $w$ for the grammar $A$. For
...
\steven{TODO define general inductives as the least fixed point of strictly
  positive endofunctors on $U$}

\steven{TODO linear $\mu$ in the type formation rules? nonlinear $\mu$ in the
...
\begin{figure}
  ...
  \end{mathpar}
  \caption{Non-linear typing (selection)}
  \label{fig:non-linear-types}
\end{figure}

\subsection{Non-linear Typing}
The non-linear types of \theoryabbv include the standard dependent
types for $\Pi$,$\Sigma$, extensional equality, unit and empty types, and
inductive types that are defined strictly positively. We may also assume function
extensionality\cite{Hofmann_1997}; Function
extensionality is holds in Cubical Agda, and in our implementation we make heavy use of it of when
validating that the denotational semantics respects the equational theory of
\theoryabbv. However, it is not necessary to assume function extensionality for
the non-linear types in \theoryabbv, as we don't utilize it in any of our constructions.

We present a selection of
the non-linear type constructions in
\Cref{fig:non-linear-types} and provide the rest in
\cref{fig:full-non-linear-types, fig:jdg-eq-nonlinear} of the appendix.
First, we include universe types $U$ of
...
numbers. As in \cite{krishnaswami_integrating_2015}, the extensional equality type
$M =_{X} N$ used here has the introduction form $refl$, but no
elimination due to the presence of equality reflection. Next, we include a non-linear type $\ltonl A$ where $A$ is a
...
formation in \cref{fig:formation} and the definition of linear terms in
...
\label{ax:dist}
For any $A : (x : X) \to Y x \to L$, the function

\[
 \lamblto{e}{\letin{\sigma\,f\,e'}{e}{\withlamb{x}{e'\,.\pi\,x}}} : \ltonl {\left(  \LinSigTyLimit{f}{\prod\limits_{x : X} . Y (x)}{\LinPiTyLimit{x}{X}{A\,x\,(f\,x)}} \lto
\LinPiTyLimit{x}{X}{\LinSigTyLimit{y}{Y(x)}{A\,x\,y}} \right)}
\]

has an inverse.
\end{axiom}

\steven{I'm not sure if this is the proper way to cite the corollary, especially
because the citation is a reference, not the originator of the idea}
\begin{corollary}[\cite{Cockett_1993}]
  \label{cor:binary-mono}
  Distributivity implies that the coprojections $\inl : A \to A \oplus B$,
  $\inr : B \to A \oplus B$ of a binary sum are monomorphisms --- i.e. if
...
\(
A \cong \left(A \& I \right) \oplus \LinSigTyLimit{c}{\Sigma_0}{\left( A \& \left( \literal{c} \otimes \top \right) \right)}
...
$\sigma~x'~a'$ of type $\LinSigTyLimit{x}{X}{A\,x}$ are not equal whenever $x \neq x'$.
...
%% our axiom about $\StringGram$ says that $\StringGram$ is a retract of $\top$.
...
$\CharGram$ --- and thus each literal
$\literal c$ --- are unambiguous as well.

% \paragraph{A Grammar of Strings}
% Define the character grammar $\CharGram$ to be the disjunction over characters in the alphabet $\Sigma$,
% $\CharGram = \LinSigTy {c} {\Sigma} c$. Then define a linear type of strings as a
% linear list of characters,
% $\StringGram = \CharGram^{*}$.

% In the grammars model, we can build a
% strong equivalence between $\top$ and $\StringGram hen building parsers, we may wish to enforce this
% syntactically by including $\top \cong \StringGram$ as an axiom.

% Through inclusion of this axiom, $! : \StringGram \to \top$ is an isomorphism, and
% thus also a monomorphism. That is, $\StringGram$ is unambiguous.
...
% $\top \to A \oplus B$. Taking $\top \cong \StringGram$, a grammar $A$ is parseable
...
Writing a parser as a linear term is an intrinsic
...
the disjointness property of \cref{def:parser} we then also get the \emph{completeness} of
...
derive unambiguity. That is, first we
construct a function $\pparse_{D}$ that is a parser for
$\Trace_D~{s}~{true}$, with $\Trace_D~{s}~{false}$ being the disjoint
type used. The fact that these trace types are disjoint follows from the
...
\newcommand{\y}{\texttt{y}}
\begin{proof}
  Define the states of $\D$ to be the $\mathbb{P}_{\epsilon}(\N.\states)$ --- the type
...
inductive hypothesis and build a parse transformer
\[
  \DtoN : \ltonl {\left( \Trace_{\D}~\X~\true \lto \bigoplus\limits_{s : \N.\states}\bigoplus\limits_{\texttt{sInX} : \X \ni \s} \Trace_{\N}~s~\true \right)}
\]
...
\theoryabbv.
\newcommand{\R}{\texttt{R}}
\begin{construction}[Thompson's Construction, \Agda]
  \label{cons:thompson}
...
equivalance of \cref{cons:determinization} to show that $\R$ is weakly
...
``stack''. The ``opening'' state $O$ expects either an left paren, in
...
data O : Nat (*@$\to$@*) Bool (*@$\to$@*) L where
  ...
  num : (*@$\uparrow$@*)(&[ n ] &[ b ] NUM (*@$\lto$@*) D n b (*@$\lto$@*) O n b)
  unexpected : (*@$\uparrow$@*)(&[ n ] (NotStartsWithLP (*@$\lto$@*) O n false)
data D : Nat (*@$\to$@*) Bool (*@$\to$@*) L where
  lookAheadRP : (*@$\uparrow$@*)(&[ n ] &[ b ] ((')' (*@$\otimes$@*) (*@$\top$@*)) (*@$\&$@*) C n b) (*@$\lto$@*) D n b)
  lookAheadNot : (*@$\uparrow$@*)(&[ n ] &[ b ] (NotStartsWithRP (*@$\&$@*) A n b) (*@$\lto$@*) D n b)
data C : Nat (*@$\to$@*) Bool (*@$\to$@*) L where
  closeGood : (*@$\uparrow$@*)(&[ n ] &[ b ] ')' (*@$\lto$@*) D n b (*@$\lto$@*) C (n + 1) b)
  closeBad : (*@$\uparrow$@*)(')' (*@$\lto$@*) C 0 false)
  unexpected : (*@$\uparrow$@*)(&[ n ] NotStartsWithRP (*@$\lto$@*) C n false)
data A : Nat (*@$\to$@*) Bool (*@$\to$@*) L where
  doneGood : (*@$\uparrow$@*)(A 0 true)
  doneBad : (*@$\uparrow$@*)(&[ n ] A (n + 1) false)
  add : (*@$\uparrow$@*)(&[ n ] &[ b ] '+' (*@$\lto$@*) O n b (*@$\lto$@*) A n b)
  unexpected : (*@$\uparrow$@*)(&[ n ] ('(' (*@$\oplus$@*) ')'(*@$\oplus$@*) NUM) (*@$\lto$@*) (*@$\top$@*) (*@$\lto$@*) A n false)
\end{lstlisting}
...
correspond to $P$.
%% Define $\StringSem$ to be the non-linear list type over the alphabet $\Sigma$ and
...
\[
Reify~P = \LinSigTyLimit {w} {\StringSem} {\LinSigTyLimit {x} {P~w} {~\lceil w \rceil}}
\]
...
transformers and verification of the equational axioms --- as well as
\cref{ax:dist,ax:disjointness,ax:string-top} --- is a relatively
...
a function $\StringSem \to \Set$. Then we implement each of the type and
...
Our design of \theoryabbv extends this view of grammars as types to a much broader class of grammars, as well as providing a syntax and equational theory for \emph{parse transformers}, showing that the terms can be viewed not just as parse trees, but as intrinsically verified parsers.

Elliott \cite{elliottSymbolicAutomaticDifferentiation2021} uses essentially the same denotational semantics as ours, interpreting regular expressions as type-valued functions on strings. Our denotational semantics of \theoryabbv extends this to a much broader class of grammars.
...
\citet{krishnaswami_integrating_2015}, itself an extension of Benton and
Wadler's linear-non-linear calculus \cite{bentonMixedLinearNonlinear1995}. A
distinct feature of these systems is that linear types and non-linear types are
distinct sorts, so the linear logic $!$ operator is not a primitive, but is
instead definable. Vakar develops a dependent linear type theory where instead
the non-linear types are accessed using the $!$ modality, similar to Girard's
original approach \cite{vakar,girard_linear_1987}. Additionally, Vakar develops
a general categorical semantics for their system, whereas we have only developed
a single denotational model. It appears straightforward to adapt Vakar's general
...
\steven{
  Is any of this detail useful here, or nah?

  They
  use their system in the design of a linear-time algorithm for greedy regular
  expression matching. It is yet unclear if greediness can be encoded in
  \theoryname; however, it appears that our approach in determinizing an NFA is
  quite similar to their algorithm. The biggest difference being that our
  approach uses a global disambiguation strategy while theirs is a local, greedy strategy.
}

We also aim to adapt the approach used for our context-free grammar parserss for
$\LLL$ and $\LRR$/$\LALRR$ with the aim of developing a shared library of intrinsically
...
{(A \lto \bigoplus_{\_:X} \top)}$. That is, a semantic action is a function that
...
\begin{figure}
  ...
  \footnotesize
    ...
    %       {\nonlinterm {\Gamma} {elim_{Nat}(X,M_0,M_{suc})(n)} {X(n)}}
...
\begin{figure}
  ...
  \inferrule{\nonlinterm {\Gamma} {M_0} {X(0)} \and
      \nonlinterm {\Gamma, n : Nat, x : X(n)} {M_{suc}(n , x)} {X(suc~n)} \and
      \nonlinterm {\Gamma} {n} {Nat}}
      {\nonlineq {\Gamma} {elim_{Nat}(X,M_0,M_{suc})(suc~n)} {M_{suc}(n ,
...
We verify that the denotational semantics verfies each of the axioms we have
assumed.

\paragraph{\cref{ax:dist}}
\begin{theorem}
...
\begin{proof}
  ...
  \[
    \lamb {\gamma} {\lamb {w} {\lamb {p} {(\lamb {\lamb {x} {(p~x).fst}} , \lamb {x} {(p~x).snd})}}}
  \]
\end{proof}

\paragraph{\cref{ax:disjointness}}
\begin{theorem}
...
\paragraph{\cref{ax:string-top}}
\begin{theorem}
  In $\Grammar$, $\semg{!}$ has an inverse.
\end{theorem}
\begin{proof}
  Because $\semg{\top}\gamma\,w$ is a singleton set, it suffices to show that
  $\semg{\StringGram}\gamma\,w$ is likewise a singleton set for all $\gamma$ and
  $w$.

  First, we prove by induction on $w$ that $\semg{\StringGram}$ is a retract of
  $\semg{\LinSigTyLimit{w}{\StringSem}{\internalize{w}}}$. Then, again by
  induction on $w$, we show that
  $\semg{\LinSigTyLimit{w}{\StringSem}{\internalize{w}}}$ is isomorphic to
  $\semg{\top}$. Thus, $\semg{\StringGram}$ is a retract of $\semg{\top}$. In particular,
  so $\semg{\StringGram}\gamma\,w$ is a singleton, and thus
  $\semg{\StringGram}\gamma\,w \cong \semg{\top}\gamma\,w$ in $\Set$.
\end{proof}

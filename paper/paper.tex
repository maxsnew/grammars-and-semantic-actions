...
discunction and $\top$ to refer to the empty conjunction. Lastly, we
...
unabmiguous if there is at most one way to transform parses of any other grammar
$\B$ into parse of $\A$.
...
is strongly equivalent to an unambguous grammar $\B$. In this style of proof, we
...
\newcommand{\y}{\texttt{y}}
\begin{proof}
  We present a variant of Rabin and Scotts's classic powerset construction \cite{rabinFiniteAutomataTheir1959}. First, we build
  ...
  $\epsilon$-closure of $\N.\init$. Lastly define the transtion function
...
on this trace which returns a family of traces parametrized by $\epsilon$-closed
...
similar. In this case we again recursivley call $\NtoD$, except afterwards we
...
the empty string is balanced, and the $\balanced$ constrcutor builds a balanced

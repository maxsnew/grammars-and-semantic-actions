...
particular grammar formalism such as non-left-recursive grammars or LL(1) grammars \cite{lasserCoStarVerifiedALL2021,EdelmannZippy2020,danielssonTotalParserCombinators2010}.
\steven{These have restrictions on left recursion or LL(1)-ness. Is it the most accurate
  to call that a difference in formalism? It would seem that these are all CFGs
  + conditions. Maybe these aren't the best works to
  compare to if they're verifying particular algorithms}
...
derivation.\max{reverse the order of these?} \steven{the order of what?}
...
inverse. Second, we need that the different constructors of
...
by defining a notion of formal grammar and parse transformer and
...
formal \emph{language}\cite{elliott}:
\begin{definition}
...
\cite{dependent-types-as-sets}. We present the concrete descriptions
of the semantics of linear types, as well as our non-standard
non-linear types in \Cref{fig:semantics}.
The grammar for a literal $c$ has a single parse
precisely when the input string consists of the single character. The
grammar for the unit similarly has a single parse for the empty
string.  A parse of the tensor product $A \otimes B$ consists of a
\emph{splitting} of the empty string into a prefix $w_1$ and suffix
$w_2$ along with an $A$ parse of $w_1$ and $B$ parse of $w_2$. A parse
of $\bigoplus_{x:X} A$ is a pair of an element of the set $X$ and a
parse of $A(x)$, while dually a parse of $\bigwith_{x:X} A$ is a
\emph{function} taking any $x:X$ to a parse of $A(x)$. A $w$-parse of
$A \lto B$ is a function that takes an $A$ parse of some other string
$w'$ to a $B$ parse of $ww'$, and $B \tol A$ is the same except the
$B$ parse is for the reversed concatenation $w'w$.
%
...
to have initial algebras \cite{initial-algebras-of-dep-poly}.  Further
these initial algebras are readily constructed in our Agda
implementation as an inductive type of $IW$ trees which are already
available in the cubical library of Agda \cite{iw,cubical-library}.
...
monoidal categories, and is included in the
appendix\cite{linearLogicMonoidalCategory}.
...
\subsection{Typed Approaches to Grammars}

\steven{Talk about Conal Elliott's Automatic and Symbolic differentiation paper}
\steven{Talk briefly about this recent arxiv paper? https://arxiv.org/abs/2411.06358}

\subsubsection{Lambek Calculus and Categorial Grammar}

\steven{Note that lambek calculus and its extensions don't have inductives, or
  even kleene star as far as I can tell. }

\steven{Choppy notes about lambek calc:
...
Also mention here that Luo's work \cite{luoLambekCalculusDependent} seems similar at first
...
\subsubsection{Other typed approaches}
\steven{this subsection should be renamed}
...
\steven{Mention Neel's typed algebraic approach paper here. Say it is similar at
  first glance, as its parsing and it has types, but that he actually is representing
  a system for proving the LL(1) condition }
...
%%    - verified imperative implementation: Lambek logic ala separation
%%      logic?
\subsection{Relation to Separation Logic}
...
\steven{Moving away from a prototype implementation of \theoryabbv}
\steven{Mention performance}
...
\steven{Define a Kleene, Lambek, and Chomsky category}

Let $\semcat$ be a Chomsky category.

\steven{Finish the denotation of lin types figure for spf, el , map , mu}
\begin{figure}
  \begin{align*}
    \semg{I} &= I \\
    \semg{A \otimes B} &= \semg{A} \otimes \semg{B} \\
    \semg{A \lto B}  &= \semg{A} \lto \semg{B} \\
    \semg{B \tol A} &= \semg{B} \tol \semg{A} \\
    \semg{\bigoplus_{x:X} A} &= \coprod_{x:X} \semg{A} \\
    \semg{\bigwith_{x:X} A} &= \prod_{x : X} \semg{A} \\
    \semg{\uparrow A} &= Hom(I,A) \\
    \semg{\equalizer{a:A}{f}{g}} &= Eq\left( \semg{f} , \semg{g} \right) \\
    \sem{L}\gamma &= \semcat_0\\
    \sem{\SPF\,X}\gamma &= \textrm{DepPolyFunctor}(\sem{X}\gamma\times \Sigma^*,\Sigma^*)\\
    \sem{\el(F)}\gamma\,G &= \sem{F}\gamma\,G\\
    \sem{\map(F)}\gamma\,f &= \sem{F}\gamma\,f\\
    \sem{\mu A}\gamma &= \mu (\sem{A}\gamma) \tag{\Cref{thm:indexed-container}}\\
  \end{align*}
  \caption{Denotation of Linear Types}
\end{figure}
...
Here we define denotations of linear terms. Note that the denotations interpret
...
functions. Further, we demonstrate that the denotational semantics resprects the
equational theory of \theoryabbv.
...
\steven{Define generalized associators}
...
$\semg{\dlamb x e} : \semg{\Delta} \to \LinPiTyLimit {x} {X} {\sem{A}(\gamma , x)}$ is
...
\begin{tikzcd}
  ...
  \LinPiTyLimit {x} {X} {\sem{A}(\gamma , x)} \arrow[r , "\pi_M"] &
  \sem{A}(\gamma , M)
\end{tikzcd}
\end{center}
...
$\semg{\sigma~M~e} : \semg{\Delta} \to \LinSigTyLimit {x} {X} \sem{A}(\gamma, x)$

\begin{center}
\begin{tikzcd}
  \semg{\Delta} \arrow[r , "\semg{e}"] &
  \sem{A}(\gamma, M) \arrow[r , "i_M"] &
  \LinPiTyLimit {x} {X} {\sem{A}(\gamma , x)}
\end{tikzcd}
\end{center}
...
{\left( \semg{\Delta'_1} \otimes \LinSigTyLimit {x} {X} {\sem{A}(\gamma , x)} \right) \otimes \semg{\Delta'_2}} \arrow[d, "d"]                                                                 & \left( \semg{\Delta'_1} \otimes \semg{\Delta} \right) \otimes \semg{\Delta'_2} \arrow[l, "(id \otimes \semg{e}) \otimes id"] \\
{\LinSigTyLimit {x} {X} {\left( \sem{\Delta'_1}(\gamma , x) \otimes
      \sem{A}(\gamma , x) \right) \otimes \sem{\Delta'_2}(\gamma , x)}} \arrow[r, "{m^{-1}_{(\Delta'_1,a:A),\Delta'_2}}"] & {\LinSigTyLimit {x} {X} {\sem{\Delta'_1,a : A,\Delta'_2}(\gamma , x)}} \arrow[d, "{[ \sem{e'}(\gamma , x) ]_{(x : X)}}"]     \\
...
\begin{proof}
  ...
  & = {[ \sem{e'}(\gamma , x) ]_{(x : X)}} \circ m^{-1} \circ d \circ (id
  \otimes \semg{\sigma~M~a'}) \otimes id \circ m \otimes id \circ m \\
  & = {[ \sem{e'}(\gamma , x) ]_{(x : X)}} \circ m^{-1} \circ d \circ (id
  \otimes i_M \circ \semg{a'}) \otimes id \circ m \otimes id \circ m \\
  & = {[ \sem{e'}(\gamma , x) ]_{(x : X)}} \circ m^{-1} \circ d \circ (id
  \otimes i_M \circ \lambda) \otimes id \circ m \otimes id \circ m
  \end{align*}
\steven{TODO need to track subscripts on $m$ in the $\bigoplus\beta$ proof, and
  finish this part of the above part of the proof}
  ...
  \begin{align*}
    ...
    &= \semg{e'}
  \end{align*}
  \steven{TODO shouldn't the semantics of $e'$ be in context $\gamma,x$ not $x$?}
\end{proof}

\paragraph{$\bigoplus\eta$}
\steven{TODO $\eta$ for sums}

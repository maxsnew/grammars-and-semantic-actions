...
\begin{figure}
  \begin{mathpar}
    ...
    \inferrule{\lintctxwffjdg \Gamma A}{\linctxwffjdg \Gamma {\unquoteTy
        {\quoteTy A} \equiv A}}
  \end{mathpar}
...
\steven{need to write the rules for the equalities of codes
  $M \equiv \quoteTy {\unquoteTy M} : U$ in an equational theory figure for
  non-linear stuff? Same for the codes in universe $L$.

  I will put the nonlinear equational rules, which are so
  unnecessary, in the appendix}
\begin{figure}
  \begin{mathpar}
  \footnotesize
    \boxed{\nonlinterm \Gamma M X}
...
\cref{fig:full-non-linear-types} \steven{Do we all need non-linear terms? That seems
extra} of the appendix.
...
of the context, whereas the introduction rule\footnote{Rules for $\tol$ elided
  here, but found in the appendix in \cref{fig:extend-lin-terms}.} for $B \tol A$ adds a
...
\steven{cite reference for distributivity implies coprojections mono. Cockett
  ``Introduction to Distributive Categories''? Awodey?}
...
were given originally. This ensures that the elements of the $\StringGram$
...
Frisch and Cardelli introduced the use of a simple type system to reason about regular expressions up to weak equivalence \cite{frischCardelli}.
...
LL and LR/LALR with the aim of developing a shared library of intrinsically

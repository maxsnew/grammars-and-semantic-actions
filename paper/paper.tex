...
Let $\stringNL$ represent a nonlinear type of strings and let
$P : \stringNL \to U$
\steven{Or should $P$ be a semantic grammar?}

The type of $P$ very nearly describes a formal grammar in \theoryabbv. That is,
$P$ describes a mapping from strings to sets of parse trees; however, even
though $P$ captures the same grammatical data $P$ is not
a first-class linear type. We reflect this nonlinear notion of grammar as a
linear type,

\[
Reflect~P = \LinSigTyLimit {w} {\stringNL} {\LinSigTyLimit {x} {P~w} {~\lceil w \rceil}}
\]

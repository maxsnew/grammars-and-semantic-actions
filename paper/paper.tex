...
\begin{itemize}
  \item $\Gamma$ --- the tape alphabet, a superset of the input alphabet $\Sigma$
  \item $\blank : \Gamma$ --- a distinguished character that marks an empty cell on the tape
  ...
  \item $\T.\delta : \T.\states \times \Tape \times \mathbb{N} \to \T.\states \times \Tape \times \mathbb{N}$
        --- a deterministic transition function. $\T.\delta$ reads the current state, the
        current tape, and the index of the head of the tape and then returns the
        updated state, tape, and head.
\end{itemize}

\begin{figure}
\begin{lstlisting}
data Turing(*@$_{\T}$@*) : (t : Tape) (*@$\to$@*) L where
  nil : (*@$\uparrow$@*)(&[ t (*@$\in$@*) Tape ] Turing(*@$_{\T}$@*) t)
  snoc : (*@$\uparrow$@*)(&[ c (*@$\in$@*) (*@$\Sigma$@*) ] &[ t (*@$\in$@*) Tape ](Turing t (*@$\lto$@*) c (*@$\lto$@*) Turing(*@$_{\T}$@*) (push c t)))
\end{lstlisting}
...
\begin{figure}
\begin{lstlisting}
parseTape : (*@$\uparrow$@*)(String (*@$\lto$@*) (*@$\oplus$@*)[ t (*@$\in$@*) Tape ] Turing(*@$_{\T}$@*) t)
...
alphabet $\Gamma$, $\Tape = \mathbb{N} \to \Gamma$.
We define $\push~\c~\ttt : \Tape \to \Tape$ to increment the index of every
character in the tape and insert $\c$ at index $0$. To run $\T$ on
the input string $\w$, we write $\w$ to the leftmost entries of the tape and
then repeatedly transition according to $\T.\delta$. If the machine enters the
states $\T.\acc$ or $\T.\rej$, then the execution of $\T$ halts and either
accepts or rejects accordingly.

In \cref{fig:turinginductive}, we give an inductive linear type that describes
Turing machines initialized with a given tape. However, unlike the types of
traces for finite automata, the execution of a Turing machine is not described
directly by the linear type system. We simply use the type $\Turing_{\T}$ to encode
the initial state of the machine. To write the input string to the leftmost
entries of the tape, we define the parse transformer
$\parseTape : \ltonl {(\stringg \lto \LinSigTy {\ttt} {\Tape} {\Turing_{\T}~\ttt})}$
in \cref{fig:parseTape}. Once the tape has been initialized, we capture the
execution of $\T$ via the non-linear types.
...
\begin{figure}
\begin{lstlisting}
data Trace(*@$_{\T}$@*) : Bool (*@$\to$@*) T.states (*@$\times$@*) Tape (*@$\times$@*) (*@$\mathbb{N}$@*) (*@$\to$@*) U where
  accept : (*@$\forall$@*) (t : Tape) (n : (*@$\mathbb{N}$@*)) (*@$\to$@*) Trace(*@$_{\T}$@*) true (T.acc , t , n)
  reject : (*@$\forall$@*) (t : Tape) (n : (*@$\mathbb{N}$@*)) (*@$\to$@*) Trace(*@$_{\T}$@*) false (T.rej , t , n)
  move : (*@$\forall$@*) (b : Bool) (s : T.states) (t : Tape) (n : (*@$\mathbb{N}$@*)) (*@$\to$@*)
             Trace(*@$_{\T}$@*) b (T.(*@$\delta$@*) s t n) (*@$\to$@*) Trace(*@$_{\T}$@*) b (s , t , n)
\end{lstlisting}
\caption{Traces through a Turing machine as a non-linear inductive type}
\label{fig:turingtrace}
\end{figure}

In \cref{fig:turingtrace} we define $\Trace_{\T}$ as a nonlinear type indexed by
an acceptance Boolean and start configuration. A configuration of a Turing
machine is the data of the start state, start tape, and start index for the head
of tape. The
traces of $\T$ either accept, reject, or transitions as described by $\T.\delta$.

Once the input string has been loaded onto the tape, we execute $\T$ by defining
a Turing machine interpreter of the following type,

\[
  \texttt{run} : \ltonl {\left( \bigoplus\limits_{\ttt : \Tape} \Turing_{\T}~\ttt \lto \bigwith\limits_{\texttt{fuel} : \mathbb{N}} \texttt{Maybe} \left(  \bigoplus\limits_{\b : \Bool} \bigoplus\limits_{\s : \T.\states}  \bigoplus\limits_{\texttt{n} : \mathbb{N}} \bigoplus\limits_{\ttt : \Tape}  \bigoplus\limits_{\texttt{tr} : \Trace_{\T}~\b~(\s , \ttt , \texttt{n})} \Turing_{\T}~\ttt \right) \right)}
\]

$\mathtt{run}$ returns a result of $\mathtt{Maybe}$
type varying over a fuel\footnote{Putting termination issues aside, we may omit
  this fuel parameter. Although it is necessary when defining a Turing machine
  interpreter in a total language like Agda.} parameter. It takes transitions at most
$\texttt{fuel}$-many transitions in $\T$, and it returns a halting trace if one is
found or timeouts otherwise. The data of this trace is encoded in the nonlinear
index $\texttt{tr} : \Trace_{\T}~\b~(\s , \ttt , \texttt{n})$. Importantly, when
$\texttt{tr}$ is an \emph{accepting} trace then the grammar
\[
\bigoplus\limits_{\s : \T.\states}  \bigoplus\limits_{\texttt{n} : \mathbb{N}} \bigoplus\limits_{\ttt : \Tape}  \bigoplus\limits_{\texttt{tr} : \Trace_{\T}~\true~(\s , \ttt , \texttt{n})} \Turing_{\T}~\ttt
\]
describes the strings that are accepted by $\T$. Therefore, we may encode
arbitrary recusively enumerable languages within \theoryabbv.

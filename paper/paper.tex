...
\stringquote{(}, \stringquote{)}, \stringquote{+} \} \cup \{ n | n \in
\mathbb N \}$ given in \Cref{fig:dyckinductive}. This grammar is
LR(1), and the corresponding automaton (which is too complex to
include here) uses the $\&$ to guard transitions with lookahead
information \max{Probably we should at least try to fit it in or put
  it in the appendix.}

\begin{theorem}[\agdalogo]
  We construct a parser for $\mathsf{Exp}$ by showing it is weakly
  equivalent to a deterministic automaton with lookahead.
\end{theorem}
\begin{figure}
\begin{lstlisting}
data Exp : L where
  num : (*@$\uparrow$@*)(&[ n : Nat ] 'n' (*@$\lto$@*) Exp)
  parens : (*@$\uparrow$@*)('(' (*@$\lto$@*) Exp (*@$\lto$@*) ('+' (*@$\otimes$@*) Atom *) ')' (*@$\lto$@*) Exp)
\end{lstlisting}
%% data Opening : Nat -> Bool -> L where
%% data Closing : Nat -> Bool -> L where
%% data Multiplying : Nat -> Bool -> L where
%%   done : (*@$\uparrow$@*)(Multiplying 0 true)
%%   times : (*@$\uparrow$@*)(&[ n : Nat ] &[ b : Bool] '*' -o Opening n b -o Multiplying n b)
%%   unmatched : (*@$\uparrow$@*)(&[ n : Nat ] Multiplying (n + 1) false)
%%   unexpected
\caption{Binary operation expressions and a corresponding lookahead automaton}
\label{fig:dyckinductive}
\end{figure}
% \subsection{LL(1)}
% %% 4. LL(1) parser

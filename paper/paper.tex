...
\(
A \cong \left(A \& I \right) \oplus \LinSigTyLimit{c}{\Sigma_0}{A \& \left( \literal{c} \otimes \top \right)}
...
\begin{corollary}
  ...
  $\inr : B \to A \oplus B$ of a binary sum are monomorphisms.
\end{corollary}
...
\steven{need to get cref to cite this as a lemma}
We use $\sigma$-disjointness in \cref{lem:unambig-to-disjoint} to proof that
...
\begin{lemma}[\agdalogo]
  ...
  As a consequence of \steven{fix cref} \cref{cor:binary-mono}, if a binary disjunction $A \oplus B$ is unambiguous then
...
the second principle, we have that $\I$ and
$\Char$ and each literal
...
\begin{definition}
  ...
  $A$ and function $\uparrow{(\String \lto
    A \oplus A_{\neg})}$.
\end{definition}
...
\begin{lemma}[\agdalogo]
  ...
  If $\LinSigTyLimit{x}{X}{A\,x}$ is unambiguous, then for $x \neq x'$, $A\,x$ and $A,\y$ are
  disjoint.

  In particular, if the binary product $A \oplus A_{\neg}$ is unambiguous, then
  $A$ and $A_{\neg}$ are disjoint.
\end{lemma}
}

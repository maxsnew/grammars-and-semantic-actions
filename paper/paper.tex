...
Lambek's original syntactic calculus \cite{lambek58} describes a logical system
for linguistic derivations. Later, he explicitly gives semantics to his
syntactic calculus inside of a non-commutative biclosed monoidal category
\cite{lambek1988categorial}. Following Lambek's illuminating ideas, Buszkowski
gives a succinct overview of the
rich body of work studying the role of linear logic and lambda calculi in
linguistics \cite{buszkowskiTypeLogicsGrammar2003}. \steven{Is citing a survey
  like this okay?} Grammars based on logic in
this manner may go by the names Lambek calculus, typelogical grammar, or
categorial grammar. Such categorial grammars correspond to the subtheory of our
calculus generated by $\otimes$, $\lto$, and $\tol$.

While applicable to both
formal and natural languages, the existing works on categorial grammar are
frequently oriented toward giving functorial semantics to natural language in
the tradition of Montague \cite{Montague1974-MONFPS}. The Abstract Categorial
Grammar Toolkit \cite{Guillaume2024} and Grammatical Framework \cite{ranta-2011}
each mechanize aspects of Lambek calculus ; however, to the best of our knowledge,
\theoryabbv is the first mechanization designed for formal language parsing first.
Moreover, we provide a novel use of inductive and dependent types inside of a
Lambek calculus.

Luo \cite{luo} describes a Lambek calculus with dependent types. However, they
do not provide an analagous denotational semantics, so it is not clear what
exactly their semantics are. Further, their contexts of non-linear variables are
ordered, while ours are not, so they provide a distinct dependent extension
whose types do not have a one-to-one correspondence with the dependent types in \theoryabbv.

\subsubsection{Typed Grammars}
The usage of a simple type system to reason about regular expressions up to weak
equivalence was
introduced by Frisch and Cardelli \cite{frischCardelli}.  Hengelin and Nielsen
...
semantic interpretation of grammars as type-level predicates on strings. The type system of \theoryabbv extends these semantic interpretations of to a broader class
of grammars.
...
of types is very different from the semantics of \theoryabbc.
...
Our Agda prototype implementation serves as a usefule proof of concept
...
programs, and to what extent the correctness of those imperative
...
semantics resprects the equational theory of \theoryabbv. The
...
It suffices to show
\[
  \semg{\letin {\sigma~x~a} {c'} {f[\sigma~x~a/c]}} = \semg{f[c'/c]}
\]
\begin{proof}
  \begin{align*}
    \semg{\letin {\sigma~x~a} {c'} {f[\sigma~x~a/c]}}
    &= [ \sem{f[\sigma~x~a/c]} ]_{(x : X)} \circ \coprod_{x:X}(m^{-1}) \circ d \circ (id \otimes \lambda) \otimes id \circ m \otimes id \circ m
  \end{align*}
\end{proof}
\max{todo: finish}

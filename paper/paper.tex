...
e'[e/a] &= e'[a'/a][e/a]
\end{align*}
so without loss of generality we may take may take $e$ to be variable $a'$.
...
% linear function
In the semantics $\semg{- \otimes A}$ and $\semg{A \lto -}$ are left adjoint, so
for all $A$ and $B$ we have the natural isomorphism
$\phi : Hom(\semg{\Delta} \otimes \semg{A}, \semg{B}) \to Hom(\semg{\Delta}, \semg{A \lto B})$.
We use this adjunction to provide the denotation $\semg{\lamblto a e} : \semg{\Delta} \to \semg{A \lto B}$.

\[
\semg{\lamblto a e} = \phi\left( \semg{e} \right)
\]

The denotation for application of a $\lto$-function,
$\semg{\applto {e'} {e}} : \semg{\Delta,\Delta'} \to \semg{B}$ defined by the diagram
...
$\semg{\lambtol a e} : \semg{\Delta} \to \semg{B \tol A}$ defined using the
natural isomorphism
...
\begin{tikzcd}
  ...
  \semg{A} \otimes \semg{\Delta} \arrow[r , "\phi^{-1}\left( \semg{e} \right)"] &
  \semg{B}
\end{tikzcd}
...
Product elimination, $\semg{e.\pi~M} : \semg{\Delta} \to \sem{A}(\gamma , M)$
...
{\left( \semg{\Delta'_1} \otimes \LinSigTyLimit {x} {X} {\sem{A}(\gamma , x)} \right) \otimes \semg{\Delta'_2}} \arrow[d, ""]                                                                 & \left( \semg{\Delta'_1} \otimes \semg{\Delta} \right) \otimes \semg{\Delta'_2} \arrow[l, "(id \otimes \semg{e}) \otimes id"] \\
...
Equalizer elimination, $\semg{\equalizerpi e} : \semg{\Delta} \to \semg{A}$

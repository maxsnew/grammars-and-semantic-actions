...
\newcommand{\fillerlink}{http://localhost:1313}
\newcommand{\Agda}[1]{\href{#1}{\agdalogo}}
...
automata. Throughout this paper, \agdalogo~ will mark
...
% The core idea of the denotational semantics is
...
\begin{itemize}
  ...
  \item A demonstration of how to encode many common grammar formalisms
...
\end{mathpar}
\caption{(\Agda{fillerlink}) \stringquote{ab} is parsed by $(\literal a \oplus \literal b) \otimes \literal c$
}
\label{fig:fingram}
\end{figure}
...
\end{lstlisting}
\caption{(\Agda{\fillerlink}) \stringquote{ab} is parsed by
  $({\literal a}^{*} \otimes \literal b) \oplus \literal c$}
...
explicit recursive call in the body of its definition. At the bottom of
\cref{fig:kleeneabstractproof}, we give a defintion of the same function as it
appears in our current implementation. $h'$ is written via combinators,
without any pattern matching or introduction
of named linear variables. Further, when written in this style, we must
explicitly invoke combinators that explicitly reassociate $\otimes$. \steven{What else to say here?}
...
\begin{lstlisting}
h' : (*@$\uparrow$@*)((A (*@$\otimes$@*) A)(*@$^*$@*) (*@$\lto$@*) A(*@$^*$@*))
h' = fold*r nil (cons (*@$\circ$@*) id (*@$\otimes$@*) cons (*@$\circ$@*) assoc(*@$^{-1}$@*))
\end{lstlisting}
\caption{(\Agda{\fillerlink}) A parse transformer for abstract grammars}
...
\end{lstlisting}
  ...
  \end{tikzpicture}
  \end{minipage}%
  \caption{(\Agda{\fillerlink}) NFA for $(\a^{*} \otimes \b) \oplus \c$ and its corresponding type}
...
\[
 \lamblto{e}{\letin{\sigma\,f\,e'}{e}{\withlamb{x}{e'\,.\pi\,x}}} : \ltonl {\left(  \LinSigTyLimit{f}{\PiTy{x}{X}{Y(x)}}{\LinPiTyLimit{x}{X}{A\,x\,(f\,x)}} \lto
...
parse(*@$_D$@*) (String.cons ((*@$\sigma$@*) c a) w) s = let (*@$\sigma$@*) b t = parse w (D.(*@$\delta$@*) c s) in
...
\begin{construction}[Determinization, \Agda{\fillerlink}]
...
\theoryabbv.
\newcommand{\R}{\texttt{R}}
\begin{construction}[Thompson's Construction, \Agda{\fillerlink}]
  \label{cons:thompson}
...
%% The first two equalities are by definition. The third equality holds because
...
\begin{construction}[\Agda{\fillerlink}]
  \label{cons:turing}
...
variables in linear terms. We give examples of this style in
\cref{fig:kleeneabstractproof} and the linked Agda code \agdalogo.
...
Wadler's linear-non-linear calculus \cite{benton1994}. A
...
instead definable, for instance in \theoryabbv as $!A = \LinSigTy{\_}{\ltonl A}{I}$.
V\'ak\'ar develops a dependent linear type theory where instead the non-linear
...
We also aim to adapt the approach used for our context-free grammar parsers for
...
\steven{I added rules for Bool, Nat, and List. What else do we need?}

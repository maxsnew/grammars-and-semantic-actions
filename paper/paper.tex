...
invest a significant number of person-hours into proof development. In early
versions of CompCert, the apparent
assurances associated with their effort were at risk of invalidation by
...
it is still arduous and highly nontrivial to write a verified
parser. Further, approaches are sensitive to a choice of grammar and automata formalism to serve
as a
foundation, which can make it difficult to relate and interoperate different
styles of verified parsers.

In \cite{elliottSymbolicAutomaticDifferentiation2021}, Elliott describes formal
grammar as type-level predicates on strings that prove
...
showing that $w$ belongs to the language recognized by $A$. This typed approach
to parsing serves a general setting that can simultaneuously interpret many distinct
grammar formalisms. In this paper, we adopt Elliott's view on formal grammars and present a
syntax that axiomatizes this semantic notion of grammar.

We present \theoryname (\theoryabbv) as a domain specific
...
them familiar to write and amenable to compositional verification techniques. We
implement \theoryabbv as a shallowly embedded domain-specific langauge in the
Cubical Agda proof assistant. Throughout this paper, we will use \agdalogo~ to
mark results that are mechanized in our Agda development.

\paragraph{Contributions} This paper begins in \cref{sec:type-theory-examples}
by studying small example programs from \theoryabbv to build intuition.
...
theory. Later in \cref{sec:applications} we demonstrate the
applicability of \theoryabbv for relating familiar grammar and automata
formalisms as well as building concrete parsers.
...
\begin{itemize}
  ...
  \item A denotational grammars semantics for \theoryabbv
  \item An implementation of \theoryabbv in Agda
\end{itemize}
...
grammars (CFGs). An interesting nontrivial CFG is the Dyck grammar

...
for linguistic derivations, and it can be given semantics inside of a non-commutative biclosed monoidal category
\cite{lambek1988categorial}. This led to many uses of non-commutative linear logic and lambda calculi in
linguistics \cite{buszkowskiTypeLogicsGrammar2003} --- including mechanized
categorial grammar parsers \cite{Guillaume2024,ranta-2011}. This style of grammar
formalism has gone by the names Lambek calculus or \emph{categorial grammar},
and it is equal in expressivity to context-free grammars.
The existing works on categorial grammar
frequently present logical operations on linear predicates rather than the term
language given in this paper.
Additionally, much of the existing work is
oriented toward computational linguistics, giving functorial semantics to natural language in
the tradition of Montague \cite{Montague1974-MONFPS}.

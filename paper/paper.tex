...
\steven{The axioms that I think we need:
...
\begin{gather}
  ...
  c \lto c' \cong 0, c \neq c' \\
  c \lto I \cong 0 \\
  c \lto g \otimes h \cong \left( (c \lto g) \otimes h \right) \oplus \left( (g \amp I) \otimes (c \lto h) \right) \\
  I \lto c \cong c \\
  c \lto (g \oplus h) \cong (c \lto g) \oplus (c \lto h) \\
  c \lto (c' \lto g) \cong (c' \otimes c) \lto g
\end{gather}

For point $(5)$, Brzozowski has $I \lto g \cong g$ for all grammars $g$, but I don't think that holds for us because we want to preserve ambiguity. We could instead have something like $I \lto g \cong (g \amp I)^{*} \otimes g$. For our purposes the character presentation seems sufficient, as the derivative reasoning Brzozowski uses only takes derivatives with respect to strings.

We want $c \lto g^{*} \cong (g \amp I)^{*} \otimes (c \lto g) \otimes g^{*}$, but this is derivable utilizing axiom $(4)$ and $g^{*} \cong I \oplus (g \otimes g^{*})$.

To make the DFA proof go through I think we additionally need the map $c \otimes (c \lto A) \vdash (c \otimes \top) \amp A$ to be an iso. Specifically, this term needs an iso $\phi : c \otimes (c \lto A) \to (c \otimes \top) \amp A$ defined below,

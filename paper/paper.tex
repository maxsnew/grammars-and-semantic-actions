...
Though Kleene categories are expressive enough to reason about concepts
that are outside of reach of Kleene algebras, their simply-typed nature
makes them not so expressive from a type theory point of view.

Since we are interested in design a type theory for reasoning about various
properties of parsers, we will assume that our Kleene category is defined
internal to a model of linear depedendently typed theory, which leads us
to the notion of models we are interested in:

\begin{definition}
  A model is a locally Cartesian category with \pedro{add the linear logic connectives here} equipped with an internal Kleene category.
\end{definition}
...
\begin{align*}
  ...
  c\tol I &\equiv 0\\
  c\tol 0 &\equiv 0\\
  c\tol (A \otimes B) &\equiv (c\tol A) \otimes B + \nu(A) \otimes (c\tol B)\\
  c\tol A^* &\equiv (c\tol A) \otimes A^*\\
  c\tol (A + B) &\equiv (c\tol A) + (c\tol B)
\end{align*}

\pedro{The type family $\nu$ above is equal to $I$ if its argument
  parses the empty string and zero otherwise. In order to define
  it we should probably add an extra axiom, right?}
\steven{$\nu$ can be derived using $\amp$, $\nu(A) = I \amp A$}

\steven{In addition to Pedro's axioms:

$c, c'$ characters. $g$ any grammar.

\begin{gather}
  I \lto c \cong c \\
  c \lto (c' \lto g) \cong (c' \otimes c) \lto g
\end{gather}

For axiom $(1)$, Brzozowski has $I \lto g \cong g$ for all grammars $g$, but I don't think that holds for us because we want to preserve ambiguity. We could instead have something like $I \lto g \cong (g \amp I)^{*} \otimes g$. For our purposes the character presentation seems sufficient, as the derivative reasoning Brzozowski uses only takes derivatives with respect to strings.

Axiom $(2)$ might be admissible, but I'm unsure.

I think for Kleene star we need $c \lto g^{*} \cong (g \amp I)^{*} \otimes (c \lto g) \otimes g^{*}$. This should derivable utilizing the product axiom and $g^{*} \cong I \oplus (g \otimes g^{*})$, or is that sort of isomorphism opaque to us right now.

To make the DFA proof go through I think we additionally need to axiomatically make the map $c \otimes (c \lto A) \vdash (c \otimes \top) \amp A$ to be an iso. Specifically, this term needs an iso $\phi : c \otimes (c \lto A) \to (c \otimes \top) \amp A$ defined below,

\[
p : c \otimes (c \lto A) \vdash \phi(p) = \letin {p_{1} \otimes p_{2}} p {(p_{1} \otimes \top, p_{1}^{\lto}p_{2})} : (c \otimes \top) \amp A
\]

but this last bit isn't just Brzozowski's definitions
...
\steven{Somewhere mention that $\epsilon$ corresponds to $I$, characters $c \in \Sigma$ correspond to grammars $c$, etc}
...
denotes an $\epsilon$-transition. A
...
\[
  ...
  }{\Delta \vdash s_{\oplus}(e) := \mathsf{mfold}(e) : g \oplus g'}
\]
...
\[
  \inferrule
  {
    ...
    \Gamma ; x_{j} : \simulsubst {X_{j}}{\gamma} = X_{j}' \vdash x_{j} : X_{j}', j \not \in \{s, e\} \\
    \Gamma ; x_{e} : \simulsubst {X_{e}}{\gamma} = I \vdash x_{e} : I = X_{e}'
  }
  {\Gamma ; \Delta \vdash \mathsf{mfold}(u) : NFA(g \oplus g')}
\]
...
\]
\[
  \hat{Y_{0}} ::= c \otimes Y ~|~ Y ~|~ \hat{Y_{0}} \oplus \hat{Y_{0}}, Y \in \{Y_{s}, Y_{1}, \dots, Y_{N}\}
\]

A state $Y_{j}$ in a DFA is accepting if and only if $\hat{Y}_{j}$ contains $I$ as a summand.

some fluff to lead into the thoerem
...
\begin{proof}
  \steven{This is currently word salad}

  DFA construction

  $DFA(g) \vdash g$ : just fold over the defs in $DFA(g)$ and have every $Y_{w}$ eliminate to $w \lto g$. This is doable bc when eliminating $Y_{w'}$ you will hold terms of type $a \otimes (w \lto g)$. These $w \lto g$ terms might not be exactly what you want, but they are weakly equivalent to some derivative that you do want! How do you know that they're weakly equivalent? if $Y_{w'}$ transitions to $Y_{w}$ via character $a$, then we know that $a \lto (w \lto g)$ is weakly equivalent to $a \lto (w' \lto g)$ \emph{by construction of the DFA}. This argument entirely hinges on the method by which we defined the DFA

  $g \vdash DFA(g)$
  Idea is to us $\top \cong \text{Char}^{*}$ and $A^{*} \cong \Sigma(n) A^{\otimes n}$ and $A \cong \top \amp A$ to turn a parse into a pair of a parse and a finite string. And then this finite string can be walked through the DFA to read off the parse, and you can show its accepting. Details currently a little shaky, and encapsulated in a bad picture of a whiteboard. \steven{Gonna has this out with Max in meeting}
\end{proof}
...
\begin{proof}
  ...
  \steven{There a weird spacing issues between these align envs. Maybe wrap these in a figure env so that spacing is better}
...
\steven{This first line is a weird alignment fix, touch up}
\begin{align*}
  s_{\oplus}(r_{\oplus}(p))
  &
  = s_{\oplus}(r_{\oplus}(p)) \\
...
The map $r_{\otimes}$ in the other direction is given as,

\[
  \inferrule{
    p : g \otimes g' \vdash p : g \otimes g' \\
    p_{1} : g, p_{2} : g' \vdash \mathsf{cons}(\phi(p_{1}), \psi(p_{2})) : NFA(g \otimes g')}
  {p : g \otimes g' \vdash \letin {p_{1} \otimes p_{2}} {p} {\mathsf{cons}(\phi(p_{1}), \psi(p_{2}))} : NFA(g \otimes g')}
\]

We show that $r_{\otimes}$ and $s_{\otimes}$ form an isomorhic pair,
\steven{This proof doesn't exactly work}

\begin{align*}
r_{\otimes}(s_{\otimes}(e))
  & =_{\mu \eta} r_{\otimes}(s_{\otimes}(\mathsf{cons}_{\mu X_{s}'}(e'))) \\
  & = r_{\otimes}(\mathsf{mfold}(\mathsf{cons}_{\mu X_{s}'}(e'))) \\
  & =_{\mu \beta} r_{\otimes}(\phi^{-1}(u) \otimes \psi^{-1}(v)) \\
  & =_{\mu \beta} \mathsf{cons}(\phi \phi^{-1} u, \psi \psi^{-1} v) \\
  & =_{\mu \eta} e
\end{align*}
...
\begin{align*}
  ...
  & = s_{\otimes}(\mathsf{cons}(\phi p_{g}, \psi p_{g'})) \\

...
\]
where $\sigma$ is the substitution that unrolls the mutually recursive definition one level by replacing each $X_{k}$ with the fixed point $\mu(X_{j} = A_{j}).X_{k}$,
...
return a term of type $g \oplus g'$. The structure of such a derivation instatiates the above elimination rule as follow,
...
This finshes the construction of a parse transformer from $NFA(g \oplus g')$ to $ g \oplus g'$.
The map in the other direction is given below in a couple stages. First, we show how to turn a parse of $NFA(g)$ into a parse of $NFA(g \oplus g')$.

\[
  \inferrule
  {
    \Gamma ; \Delta \vdash u : NFA(g) = \mu(X_{j} = \hat{X}_{j}).X_{s} \\
...
\]
where the dots in the above derivation take on the mutually recursive defintions present in $NFA(g \oplus g')$. \steven{Need a better way to display this without blowing up the definitions everywhere} Denote the above embedding of $NFA(g)$ into $NFA(g \oplus g')$ via $\alpha(u) := \mathsf{mfold}(u) : NFA(g \oplus g')$. Similarly define an embedding $\beta$ for an embedding of $NFA(g')$. We can then define the derivation from $g \oplus g'$ to $NFA(g \oplus g')$ by composing the maps isomorphisms $g \overset{\phi}{\cong} NFA(g)$ and $g \overset{\psi}{\cong} NFA(g')$ with $\alpha$ and $\beta$, respectively.
...
\[
  ...
  {
    p : g \oplus g' \vdash \textbf{case}~p \{
...
By breaking into a case distinction on $e$ and then applying the $\eta$-rule for coproducts, the above proof will
simplify into two cases where $\phi$ will be inverted by $\phi^{{-1}}$ and $\psi$ will be inverted by $\psi^{-1}$,
ultimately cancelling to give the identity map. The other direction below simplifies in a similar manner.
\steven{Most concise list for this?}
...
This concludes the disjunctive combination of two NFAs.
Because they follow a very similar style of argumentation, we include the cases for sequencing and
Kleene star in \cref{sec:appendixnfa}.

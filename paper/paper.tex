...
any memory --- i.e.\ finite state machines.
...
\steven{Need to add argument that justifies why the $X_{j}$s are allowed to be eliminated via the identity. A priori they could be interfered with by taking an $X_{s}$ that needs to be eliminated. I think it hinges on a syntactic restriction where there are no backedges to $X_{s}$ in the NFA.Even if there were, I think I can get away with a case distinction entailment instead of the one written}
...
\steven{this section sounds awkward and will be adjusted in a later editing pass}
...
Now we wish to build a derivation from $g$ to $DFA(g)$.
\steven{fill in}
\end{proof}
...
\steven{Forms for PDAs and Turing Machines go here}
...
\steven{This proof also needs more work. Esp after 4th equality}
\steven{Think I need to thread through a case distinction on what the $g^{*}$ term is. Putting a pin in this for now to type up the DFA stuff before meeting}
\begin{align*}
  r_{*}(s_{*}(e))
...
\steven{for the empty case above, can I make an $\eta$ equality for all terms of type $I$ to argue $\textbf{in}(v) = \textbf{in} (())$}

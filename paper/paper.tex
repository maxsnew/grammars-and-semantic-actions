...
with two proof terms in our logic that mutually invert each other. To define a map $NFA(g \oplus g') \to g \oplus g'$
we will use a multi-fold over all the mutually recursive defintions of $NFA(g \oplus g')$. Note, even though multiple
fixed points are not taken as primitive, using Beki\'c's theorem we can define admissible multiple fixed points types. Below we give rules to form $\mathsf{cons}$ and $\mathsf{mfold}$ terms to serve as the introduction and elimination rules, respectively, for multi-term fixed points,
...
\]
where $\sigma$ is the substitution that unrolls the mutually recursive definition one level by replacing each $X_{k}$ with the fixed point,
\steven{I'm not sure this is right. TODO sanity check it with a small example}
\begin{align*}
...
return a term of type $g \oplus g'$. The structure of such a derivation is given below,
...
The map in the other direction is given the derivation,

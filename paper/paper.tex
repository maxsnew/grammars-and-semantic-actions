...
Intuitively, this can be thought of like a directed graph with nodes in $Q$ with an edge $q \overset{c}{\to} q'$ whenever $q' \in \delta(q, c)$. Note that transitions in an NFA may be labelled with the empty string $\epsilon$.
...
mutual\footnote{For convenience we refer to a mutual fixed point here, even though we
  only take single fixed points as primitive. As in Beki\'c's theorem, we may decompose our mutual fixed points into a series of single fixed points.}
...
\steven{I'm not sure the cons rule is right. TODO sanity check it with a small example}

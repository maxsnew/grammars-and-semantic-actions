...
The main benefit of writing such a parser as a linear term, is that it
looks just like an ordinary functional program into a sum type. We
typically construct it by doing a fold over $\String$, which as we
recall is defined as a type of linear lists of characters. But we get
the \emph{soundness} of the parser completely for free from the
typing: any $\inl$ parse that we return must correspond to a parse
tree of the input string. Further if we verify the disjointness
property we then also get the \emph{completeness} of the parser as
well, that it never fails to generate an $A$ parse when it is
possible.

Our main proof tool then for constructing verified parsers is to show
that a grammar $A$ is weakly equivalent to a grammar for a
deterministic automaton. The parsers for deterministic automata can
simply be implemented by stepping through the states of the
automaton. The key is then the following simple observation:
\begin{lemma}[\agdalogo]

...
\ltonl{(\top\lto \String)}$ that says that we can extract the current
string from the empty additive conjunction. Further, we add an axiom
that this function is an isomorphism, i.e. that it is inverse to the
unique function $\ltonl{(\String \lto \top)}$. In the next section, we
will show how these basic principles are enough to provide a basis for
verified parsing and formal grammar theory.

...
Here we extend the denotational semantics from \cref{sec:semantics-and-metatheory} to cover all of \theoryname syntax.
...
{\coprod_{x : X} {\left( \sem{\Delta'_1}(\gamma , x) \otimes
      \sem{A}(\gamma , x) \right) \otimes \sem{\Delta'_2}(\gamma , x)}} \arrow[r, "{m^{-1}_{(\Delta'_1,a:A),\Delta'_2}}"] & {\coprod_{x : X} {\sem{\Delta'_1,a : A,\Delta'_2}(\gamma , x)}} \arrow[d, "{[ \sem{e'}(\gamma , x) ]_{(x : X)}}"]     \\
...
\begin{proof}
  ...
  & = ? \\
  & = {[ \sem{e'}(\gamma , x) ]_{(x : X)}} \circ i_{M} \\
  & = \sem{e'}(\gamma , x)
  \end{align*}

  \steven{How does the distributivity morphism handled? Seems like its a barrier to invoking coherence}

...
\begin{mathpar}
  \inferrule
  {
    ...
    \inferrule
    {
      ...
      {a' : \A \otimes \A , a_{*} : \A^* \vdash g := \letin {(a_1 , a_2)}
...
We present a selection of the non-linear type constructions in
\Cref{fig:non-linear-types} and provide the rest in
\cref{fig:full-non-linear-types, fig:jdg-eq-nonlinear} of the appendix.  First,
...
numbers. 
Next, we include standard $\Sigma,\Pi$, empty, unit and natural number types.
...
undecidable because the conversion rule may require an arbitrarily complex
...
not raise any issues \cite{Hofmann_1997}.
...
The following corollary is a well known consequence of distributivity \cite{Cockett_1993}.
\begin{corollary}
...
instead definable, for instance in \theoryabbv as $!A = \bigoplus_{\_:\ltonl A}
I$. Vakar develops a dependent linear type theory where instead the non-linear
types are accessed using the $!$ modality, similar to Girard's original approach
\cite{vakar,girard_linear_1987}. Additionally, Vakar develops a general
categorical semantics for their system, whereas we have only developed a single
denotational model. It appears straightforward to adapt Vakar's general
...
\max{TODO: include the rest of the rules}
\begin{figure}
  \begin{mathpar}
  \footnotesize
    \boxed{\nonlinterm \Gamma M X}

...
%DIF ADD paper.tex        Tue Mar 25 12:47:33 2025
%DIF 2c2-36
...
numbers. 
Next, we include \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{standard $\Sigma,\Pi$, empty, unit, Boolean, and natural number types.
}
...
not raise any issues \mbox{%DIFAUXCMD
\cite{Hofmann_1997}}\hskip0pt%DIFAUXCMD
.
}
...
function type
$\StringSem \to \hSet~\ell$}\DIFaddend . Then we implement each of the type and
...
, itself an extension of Benton and
Wadler's linear-non-linear calculus \mbox{%DIFAUXCMD
...
instead definable, for instance in }\theoryabbv \DIFadd{as $!A = \LinSigTy{\_}{\ltonl A}{I}$.
...
%DIFDELCMD < %%%
\DIFdelend \bibliography{refs.bib}
...
\DIFaddbegin \DIFadd{\max{TODO: include the rest of the rules}
...
\DIFaddend 
  ...
  \end{mathpar}
  \caption{\DIFadd{Non-linear Typing}}
...
\DIFdelendFL \DIFaddbeginFL \boxed{
  ...
  {M} {L}}
  \DIFaddendFL \end{mathpar}
  \caption{\DIFdelbeginFL \DIFdelFL{Linear }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Judgmental equality for non-linear }\DIFaddendFL terms\DIFdelbeginFL \DIFdelFL{(extended)}\DIFdelendFL }
  \DIFaddbeginFL \label{fig:jdg-eq-nonlinear}
\DIFaddendFL \end{figure}

...
% TODO illustrate parser generator might not mathc input gramar
% \begin{frame}{Validation of the CompCert Parser}
%   CompCert parser rewritten and validated \textbf{(Jourdan et al, 2012)}
%   \begin{itemize}
%     \item<2-> Given a grammar and an LR(1) parser, validate that they agree
%     \item<3-> Validator proven correct in Coq
%   \end{itemize}

%   Procedure is incomplete, and can be generalized
% \end{frame}

% TODO diagram w inputs and outputs
% TODO intrinsci verif
\begin{frame}{Contributions of This Project}
  ...
  \begin{block}{Roadmap}
    \begin{enumerate}
      ...
      \item<3-> Express parsing algorithms as terms
      \item<3-> Internalize proofs of parsing algorithm correctness
    \end{enumerate}
  \end{block}
  }
...
\begin{frame}{Formal Grammars}
  A formal grammar is usually given as a set of production rules
...
\begin{frame}{Formal Languages and Equivalence}
  ...
  \onslide<4->{
    \begin{center}
    $g$ and $g \oplus g$ are weakly, but not strongly, equivalent

    $g \oplus h$ and $h \oplus g$ are strongly equivalent
    \end{center}
  }
\end{frame}
...
\begin{frame}{Why Types?}
  ...
  \begin{itemize}
    ...
    \item<6-> Presheaves model dependent type theory
    \item<6-> Design a type theory where grammars are the intended model
  \end{itemize}
\end{frame}

\begin{frame}{Grammars as Types}
  Types are \textbf{grammars}, and terms are
  \only<1-4>{\textbf{parse trees}}\only<5->{\textbf{parse transformers}}
  ...
  \onslide<2->{
  \[
    x : a \otimes a \otimes a \vdash \mathsf{cons}(\mathsf{cons}(\mathsf{cons}(\mathsf{nil}))) : a^{*}
  \]
  }

  \onslide<3->{
  \[
    \alert<5>{x : (a \otimes b) \oplus a , y : b} \vdash p : a \otimes b^{*}
  \]
  }
...
\begin{frame}{Fixed-point Operator}
  ...
  \begin{itemize}
    \item Using only literals, $\varepsilon$, $\otimes$, $\oplus$, and $\amp$ we cannot describe infinite languages
  \end{itemize}
...
\begin{frame}{A Specification for Parsers}
  ...
  \begin{exampleblock}{Example}
    ...
    \onslide<10->{$\longrightarrow \mathsf{parse}_{g}(w) = {\color{blue} \mathsf{inl}(p_{g})} : \alert<10>{g} \oplus \top$

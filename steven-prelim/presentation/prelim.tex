...
\begin{frame}{Related Work}
  \talkenote{KA encapsulated with weak equivalence}
  \talkenote<2>{They use types for parsing but terms are grammars and types are LL(1)-ness}
  ...
  \talkenote<3>{They later give a validation procedure for CompCert}
  \talkenote<3>{incomplete validation, and limited at LR(1)}
  \talkenote<3>{LR(1) more powerful than LL(1). LR:target string back to start symbol. LL:start to target}
  \talkenote<4>{Several other works build verified parsers, but each have some caveats}
  \talkenote<4>{internalize each of these within the type theory}

...
From `code/cubical`, running `make` will compile `README.agda` which imports the entirety of the project. This may take longer than 30 minutes, as it will also compile the dependencies from `cubical` and `cubical-categorical-logic` if they are not already built. You may also build `README.agda` interactively by loading the file with [agda-mode](https://agda.readthedocs.io/en/v2.7.0.1/tools/emacs-mode.html).
...
We propose evaluating this codebase by running it through the Agda typechecker. To regenerate all the `Everything` files and check the entire project, run 
...
To achieve a similar behavior for the full syntax that is presented in the paper, we need to implement an ordered, linear typechecker. Then a user can introduce named variables and the typechecker would guarantee that they are pieced together in a manner that is coherent with the current string and grammar. 

The combinatory style has equivalent semantics to the full syntax in the paper, so in this initial implementation we have chosen to use combinators so that we may focus on encoding parsing algorithms instead of focusing on syntax.

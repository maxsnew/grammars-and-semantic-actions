...
From the `code/Cubical` directory, `make` will compile `README.agda` which imports the entirety of the project. This may take longer than 30 minutes, as it will also compile the dependencies from `cubical` and `cubical-categorical-logic` if they are not already built.
...
#### Grammar Connectives
The typing connectives of `Lambekᴰ` are given in the `Grammar` module.
...
We include the code locations of the claims made in the paper.
...
TODO update determinization to use this parser

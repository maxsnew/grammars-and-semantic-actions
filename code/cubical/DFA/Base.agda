...
record DFA : Type (ℓ-suc ℓ-zero) where
  ...
  -- module _ (q-end : ⟨ Q ⟩) where
    ...
    -- data Trace : (q : ⟨ Q ⟩) → Grammar ℓ-zero where
    --   nil : ε ⊢ Trace q-end
    --   cons : ∀ q c →
  ...
  TraceTy q = ⊕e TraceTag λ {
    stop → k ε
    ; step → ⊕e ⟨ Alphabet ⟩ λ c → ⊗e (k (literal c)) (Var (δ q c)) }

  Trace' : (q : ⟨ Q ⟩) → Grammar ℓ-zero
  Trace' = μ TraceTy
  ...
  --   RejectingTraceFrom : Grammar ℓ-zero
  --   RejectingTraceFrom =
  --     LinΣ[ q-end ∈ ⟨ Q ⟩ ]
  --       LinΣ[ acc ∈ (⟨ isAcc q-end .fst ⟩ → Empty.⊥) ] Trace' q-end q-start

  -- InitTrace : Grammar ℓ-zero
  -- InitTrace = TraceFrom init

  -- InitParse : Grammar ℓ-zero

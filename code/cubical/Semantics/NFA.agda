...
module _ {ℓN} {ℓN'} {Σ₀ : Type ℓΣ₀}
  ...
  trace⊕NFA→traceN⊕traceN' fzero = {!!}
  trace⊕NFA→traceN⊕traceN' (inr (inl qN-end)) =
    {!elim!}
    -- ⊕-intro₁
    --   {g = ⟨ ⊕NFA ⟩[ ⊕NFA .init →* inr (inl (qN-end)) ]}
    --   {h = TraceFrom N (N .init)}{k = TraceFrom N' (N' .init)}
    --   (λ trace → {!qN-e!} , {!!})

  trace⊕NFA→traceN⊕traceN' (inr (inr qN'-end)) =
    {!!}

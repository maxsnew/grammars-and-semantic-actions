...
module NFADefs ℓN (Σ₀ : Type ℓΣ₀) where
  open StringDefs {ℓΣ₀} {Σ₀}

  record NFA : Type (ℓ-suc (ℓ-max ℓΣ₀ ℓN)) where
    field
      Q : FinSet ℓN
      init : Q .fst
    ...
    acc? : Q .fst → Grammar (ℓ-max (ℓ-suc ℓN) ℓΣ₀) {Σ₀}
    acc? q = DecProp-grammar' {ℓG = ℓ-suc ℓN}{ℓN}(isAcc q)

    rej? : Q .fst → Grammar (ℓ-max (ℓ-suc ℓN) ℓΣ₀) {Σ₀}
    rej? q = DecProp-grammar' {ℓG = ℓ-suc ℓN}{ℓN} (negateDecProp (isAcc q))

    module _ (q-end : Q .fst) where
      data NFATrace : Q .fst → Grammar (ℓ-max ℓΣ₀ (ℓ-suc ℓN)) {Σ₀} where
        nil : ε-grammar {ℓG = ℓ-max ℓN ℓΣ₀} ⊢ NFATrace q-end
        cons : ∀ tr →
          literal {ℓG = ℓ-max ℓN ℓΣ₀} (label tr) ⊗ NFATrace (dst tr) ⊢ NFATrace (src tr)
        ε-cons : ∀ εtr →
          NFATrace (ε-dst εtr) ⊢ NFATrace (ε-src εtr)

      elimNFA :
        (P : ∀ q-start → Grammar ℓP {Σ₀}) →
        (nil-case : ε-grammar {ℓG = ℓ-max ℓN ℓΣ₀} ⊢ P q-end) →
        (cons-case : ∀ {tr} →
          literal {ℓG = ℓ-max ℓN ℓΣ₀} (label tr) ⊗ P (dst tr) ⊢ P (src tr)) →
        (ε-cons-case : ∀ {εtr} →
    ...
    NFATrace-syntax : Q .fst → Q .fst → Grammar (ℓ-max ℓΣ₀ (ℓ-suc ℓN)) {Σ₀}
    ...
    module _ (q-start : Q .fst) where
      data SnocNFATrace : (q-end : Q .fst) → Grammar (ℓ-max ℓΣ₀ (ℓ-suc ℓN)) {Σ₀} where
        nil : ε-grammar {ℓG = ℓ-max ℓN ℓΣ₀} ⊢ SnocNFATrace q-start
        snoc : ∀ tr →
          SnocNFATrace (src tr) ⊗ literal {ℓG = ℓ-max ℓN ℓΣ₀} (label tr) ⊢ SnocNFATrace (dst tr)
        ε-snoc : ∀ εtr →
          SnocNFATrace (ε-src εtr) ⊢ SnocNFATrace (ε-dst εtr)

      elimSnocNFA :
        (P : ∀ q-end → Grammar ℓP {Σ₀}) →
        (nil-case : ε-grammar {ℓG = ℓ-max ℓN ℓΣ₀} ⊢ P q-start) →
        (snoc-case : ∀ {tr} →
          P (src tr) ⊗ literal {ℓG = ℓ-max ℓN ℓΣ₀} (label tr) ⊢ P (dst tr)) →
        (ε-snoc-case : ∀ {εtr} →
          P (ε-src εtr) ⊢ P (ε-dst εtr)) →
        ∀ {q-end} →
          SnocNFATrace q-end ⊢ P q-end
      elimSnocNFA P nil-case snoc-case ε-snoc-case (nil x) = nil-case x
      elimSnocNFA P nil-case snoc-case ε-snoc-case (snoc tr x) =
        snoc-case ((x .fst) , ((elimSnocNFA P nil-case snoc-case ε-snoc-case (x .snd .fst)) , (x .snd .snd)))
      elimSnocNFA P nil-case snoc-case ε-snoc-case (ε-snoc εtr x) =
        ε-snoc-case (elimSnocNFA P nil-case snoc-case ε-snoc-case x)

    SnocNFATrace-syntax : Q .fst → Q .fst → Grammar (ℓ-max ℓΣ₀ (ℓ-suc ℓN)) {Σ₀}
    ...
    module _ (q-start : Q .fst) where
      TraceFrom : Grammar (ℓ-max ℓΣ₀ (ℓ-suc ℓN)) {Σ₀}
      TraceFrom = LinearΣ (λ (q-end : Q .fst) → NFA[ q-start →* q-end ])

      SnocTraceFrom : Grammar (ℓ-max ℓΣ₀ (ℓ-suc ℓN)) {Σ₀}
      SnocTraceFrom = LinearΣ (λ (q-end : Q .fst) → SnocNFA[ q-start →* q-end ])

      AcceptingFrom : Grammar (ℓ-max ℓΣ₀ (ℓ-suc ℓN)) {Σ₀}
      AcceptingFrom =
        LinearΣ
          (λ ((q-end , isAcc-q-end ): Σ[ q ∈ Q .fst ] isAcc q .fst .fst) →
             NFA[ q-start →* q-end ])

      SnocAcceptingFrom : Grammar (ℓ-max ℓΣ₀ (ℓ-suc ℓN)) {Σ₀}
      SnocAcceptingFrom =
        LinearΣ
          (λ ((q-end , isAcc-q-end ): Σ[ q ∈ Q .fst ] isAcc q .fst .fst) →
    ...
    Parses : Grammar (ℓ-max ℓΣ₀ (ℓ-suc ℓN)) {Σ₀}
...
module NFATraceSyntax (Σ₀ : Type ℓΣ₀) where

  NFATrace-syntax-rebind : ∀ {ℓN} → (N : NFA ℓN Σ₀) →
    (q-end : Q N .fst) → Q N .fst → StringDefs.String → Type (ℓ-max ℓΣ₀ (ℓ-suc ℓN))
...
module _ {ℓN} {Σ₀ : Type ℓΣ₀} where
  ...
  ε-grammar→snocEmptyNFA :
    ε-grammar
    ⊢
    ⟨ emptyNFA ⟩Snoc[ emptyNFA .init →* (lift (fsuc fzero)) ]
  ε-grammar→snocEmptyNFA =
    trans {g = ε-grammar}
     {h =
      SnocNFATrace-syntax-rebind emptyNFA (emptyNFA .init)
      (emptyNFA .init)}
     {k =
      SnocNFATrace-syntax-rebind emptyNFA (emptyNFA .init)
      (lift (fsuc fzero))}
  ...
  ε-grammar→traceFromEmptyNFA :
    ε-grammar
    ⊢
    TraceFrom emptyNFA (emptyNFA .init)
  ε-grammar→traceFromEmptyNFA pε =
    (lift (fsuc fzero)) ,
    (trans {g = ε-grammar}
      {h =
       SnocNFATrace-syntax-rebind emptyNFA (emptyNFA .init)
       (lift (fsuc fzero))}
      {k =
       NFATrace-syntax-rebind emptyNFA (lift (fsuc fzero))
       (emptyNFA .init)}
      ε-grammar→snocEmptyNFA
      (SnocNFATrace→NFATrace emptyNFA (lift fzero) (lift (fsuc fzero)))
      pε)

  private
    the-P : emptyNFA .Q .fst → Grammar (ℓ-max ℓΣ₀ ℓN) {Σ₀}
    the-P q-end = ε-grammar {ℓG = ℓN}
  ...
  traceEmptyNFA→ε-grammar q-end =
    elimNFA emptyNFA q-end
      ...
      (id {g = ε-grammar})
      (λ {tr} → ⊥.rec (lower tr))
      (λ {εtr} → (id {g = ε-grammar}))

  ε-grammar→parseEmptyNFA :
    ε-grammar
    ⊢
    Parses emptyNFA
  ε-grammar→parseEmptyNFA pε =
    (lift (inr (inl tt)) , refl) ,
    (ε-grammar→traceFromEmptyNFA pε .snd)
  ...
  Iso.leftInv (parseEmptyNFA≡ε-grammar w) a = {!!}

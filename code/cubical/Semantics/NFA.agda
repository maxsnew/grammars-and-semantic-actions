...
module NFADefs ℓN (Σ₀ : Type ℓ-zero) where
  record NFA : Type (ℓ-suc ℓN) where
    ...
    module _ (q-start : Q .fst) where
      ...
      record SnocNFATrace-Alg : Typeω where
        field
          the-ℓs : Q .fst → Level
      ...
      open SnocNFATrace-Alg

      record SnocNFATrace-AlgHom (alg alg' : SnocNFATrace-Alg) : Typeω where
        field
            ...
            Term≡ {g = alg .P (src tr) ⊗ literal (label tr)}
              ...
              (trans {g = alg .P (src tr) ⊗ literal (label tr)}
                (cut {g = alg .P (src tr)} {h = alg' .P (src tr)}
                  ...
                  (f (src tr)))
                (alg' .snoc-case tr))
          on-ε-snoc : (εtr : ε-transition .fst) →
      ...
      id-SnocAlgHom : (alg : SnocNFATrace-Alg) →
        SnocNFATrace-AlgHom alg alg
      SnocNFATrace-AlgHom.f (id-SnocAlgHom alg) q-start =
        id {g = alg .P q-start}
      SnocNFATrace-AlgHom.on-nil (id-SnocAlgHom alg) _ = refl
      SnocNFATrace-AlgHom.on-snoc (id-SnocAlgHom alg) _ _ = refl
      SnocNFATrace-AlgHom.on-ε-snoc (id-SnocAlgHom alg) _ _ = refl

      open SnocNFATrace-AlgHom

      SnocNFATrace-AlgHom-seq : {alg alg' alg'' : SnocNFATrace-Alg} →
        SnocNFATrace-AlgHom alg alg' → SnocNFATrace-AlgHom alg' alg'' →
        SnocNFATrace-AlgHom alg alg''
      f (SnocNFATrace-AlgHom-seq ϕ ψ) q-end x =
        ψ .f q-end (ϕ .f q-end x)
      on-nil (SnocNFATrace-AlgHom-seq ϕ ψ) p =
        cong (ψ .f q-start) (ϕ .on-nil p) ∙
        (ψ .on-nil p)
      on-snoc (SnocNFATrace-AlgHom-seq ϕ ψ) tr (s , p , lit) =
        cong (ψ .f (dst tr)) (ϕ .on-snoc tr (s , p , lit)) ∙
        (ψ .on-snoc tr (s , (ϕ .f (src tr) p) , lit))
      on-ε-snoc (SnocNFATrace-AlgHom-seq ϕ ψ) εtr p =
        cong (ψ .f (ε-dst εtr)) (ϕ .on-ε-snoc εtr p) ∙
        ψ .on-ε-snoc εtr (ϕ .f (ε-src εtr) p)

      initialSnocAlg : SnocNFATrace-Alg
      the-ℓs initialSnocAlg _ = ℓN
      P initialSnocAlg = SnocNFATrace
      nil-case initialSnocAlg = nil
      snoc-case initialSnocAlg = snoc
      ε-snoc-case initialSnocAlg = ε-snoc

      module _
        (the-alg : SnocNFATrace-Alg)
        where
        elimSnocNFA : ∀ {q-start} → SnocNFATrace q-start ⊢ the-alg .P q-start
        elimSnocNFA (nil x) = the-alg .nil-case x
        elimSnocNFA (snoc tr x) =
          the-alg .snoc-case tr ((x .fst) ,
            ((elimSnocNFA (x .snd .fst)) , (x .snd .snd)))
        elimSnocNFA (ε-snoc εtr x) = the-alg .ε-snoc-case εtr (elimSnocNFA x)

        ∃SnocAlgHom : SnocNFATrace-AlgHom initialSnocAlg the-alg
        f ∃SnocAlgHom q-end = elimSnocNFA {q-end}
        on-nil ∃SnocAlgHom p = refl
        on-snoc ∃SnocAlgHom tr p = refl
        on-ε-snoc ∃SnocAlgHom εtr p = refl

        !SnocAlgHom :
          (e : SnocNFATrace-AlgHom initialSnocAlg the-alg) →
          (q-end : Q .fst) →
          Term≡ {g = SnocNFATrace q-end} (e .f q-end) elimSnocNFA
        !SnocAlgHom e q-start (nil x) = e .on-nil x
        !SnocAlgHom e .(dst tr) (snoc tr x) =
          e .on-snoc tr x ∙
            cong (λ a → the-alg .snoc-case tr (x .fst , a , x .snd .snd))
              (!SnocAlgHom e (src tr) (x .snd .fst))
        !SnocAlgHom e .(ε-dst εtr) (ε-snoc εtr p) =
          e .on-ε-snoc εtr p ∙
          cong (λ a → the-alg .ε-snoc-case εtr a)
            (!SnocAlgHom e (ε-src εtr) p)

      initialSnocAlg→initialAlg≡id :
        (e : SnocNFATrace-AlgHom initialSnocAlg initialSnocAlg) →
        (q-end : Q .fst) →
        Term≡ {g = SnocNFATrace q-end}
          (e .f q-end)
          (id-SnocAlgHom initialSnocAlg .f q-end)
      initialSnocAlg→initialAlg≡id e q-end p =
        !SnocAlgHom initialSnocAlg e q-end p ∙
        sym
          (!SnocAlgHom initialSnocAlg (id-SnocAlgHom initialSnocAlg) q-end p)
    ...
    open SnocNFATrace-Alg
    open SnocNFATrace-AlgHom
    NFATrace-SnocAlg : ∀ q-start → SnocNFATrace-Alg q-start
    the-ℓs (NFATrace-SnocAlg q-start) = λ _ → ℓN
    P (NFATrace-SnocAlg q-start) q-end =
      NFA[ q-start →* q-end ] 
    nil-case (NFATrace-SnocAlg q-start) = nil
    snoc-case (NFATrace-SnocAlg q-start) tr =
       trans
        {g = NFA[ q-start →* src tr ] ⊗ literal (label tr)}
        {h = SnocNFA[ q-start →* src tr ] ⊗ literal (label tr)}
        {k = NFA[ q-start →* dst tr ]}
        (cut {g = NFATrace-syntax (src tr) q-start}
          {h = SnocNFATrace-syntax q-start (src tr)}
          (var ⊗l literal (label tr))
          (NFATrace→SnocNFATrace q-start (src tr)))
        (trans {g = SnocNFA[ q-start →* src tr ] ⊗ literal (label tr)}
          {h = SnocNFA[ q-start →* dst tr ]}
          {k = NFA[ q-start →* dst tr ]}
          (snoc tr)
          (SnocNFATrace→NFATrace q-start (dst tr)))
    ε-snoc-case (NFATrace-SnocAlg q-start) εtr =
      trans
        {g = NFA[ q-start →* ε-src εtr ]}
        {h = SnocNFA[ q-start →* ε-src εtr ]}
        {k = NFA[ q-start →* ε-dst εtr ]}
        (NFATrace→SnocNFATrace q-start (ε-src εtr))
        (trans
        {g = SnocNFA[ q-start →* ε-src εtr ]}
        {h = SnocNFA[ q-start →* ε-dst εtr ]}
        {k = NFA[ q-start →* ε-dst εtr ]}
          (ε-snoc εtr)
          (SnocNFATrace→NFATrace q-start (ε-dst εtr))
          )

    NFATrace-SnocAlgHom : ∀ q-start →
      SnocNFATrace-AlgHom q-start
        (NFATrace-SnocAlg q-start)
        (initialSnocAlg q-start)
    f (NFATrace-SnocAlgHom q-start) q-end =
      NFATrace→SnocNFATrace q-start q-end
    on-nil (NFATrace-SnocAlgHom q-start) pε = refl
    on-snoc (NFATrace-SnocAlgHom .(src tr)) tr (s , nil x , lit) = {!ΣPathP!}
    on-snoc (NFATrace-SnocAlgHom .(src tr₁)) tr (s , cons tr₁ x , lit) = {!!}
    on-snoc (NFATrace-SnocAlgHom .(ε-src εtr)) tr (s , ε-cons εtr p , lit) = {!!}
    on-ε-snoc (NFATrace-SnocAlgHom q-start) = {!!}
    -- the-ℓs NFATrace-SnocAlg _ = ℓN
    -- P NFATrace-SnocAlg q-end = NFA[ q-start →* q-end ]
    -- nil-case NFATrace-SnocAlg = nil
    -- snoc-case NFATrace-SnocAlg tr (s , nil x , lit) = {!!}
    -- snoc-case NFATrace-SnocAlg tr (s , cons tr₁ x , lit) = {!!}
    -- snoc-case NFATrace-SnocAlg tr (s , ε-cons εtr p , lit) = {!!}
    -- ε-snoc-case NFATrace-SnocAlg εtr = {!!}

    SnocNFATrace→NFATrace∘NFATrace→SnocNFATrace-AlgHom :
      ∀ q-start →
      SnocNFATrace-AlgHom
        q-start
        (initialSnocAlg q-start)
        (initialSnocAlg q-start)
    f (SnocNFATrace→NFATrace∘NFATrace→SnocNFATrace-AlgHom q-start) q-end =
      trans {g = SnocNFATrace-syntax q-start q-end}
        (SnocNFATrace→NFATrace q-start q-end)
        (NFATrace→SnocNFATrace q-start q-end)
    on-nil (SnocNFATrace→NFATrace∘NFATrace→SnocNFATrace-AlgHom q-start) pε =
      refl
    on-snoc (SnocNFATrace→NFATrace∘NFATrace→SnocNFATrace-AlgHom q-start) tr p =
      {!!}
    on-ε-snoc (SnocNFATrace→NFATrace∘NFATrace→SnocNFATrace-AlgHom q-start) =
      {!!}
    -- f SnocNFATrace→NFATrace∘NFATrace→SnocNFATrace-AlgHom q-end =
    --   trans {g = SnocNFA[ q-start →* q-end ]}
    --     {!SnocNFATrace→NFATrace!}
    --     {!!}
    -- on-nil SnocNFATrace→NFATrace∘NFATrace→SnocNFATrace-AlgHom = {!!}
    -- on-snoc SnocNFATrace→NFATrace∘NFATrace→SnocNFATrace-AlgHom = {!!}
    -- on-ε-snoc SnocNFATrace→NFATrace∘NFATrace→SnocNFATrace-AlgHom = {!!}



    -- NFATrace≃SnocNFATrace :
    --   isStronglyEquivalent (NFA[ q-start →* q-end ]) (SnocNFA[ q-start →* q-end ])
    -- Iso.fun (NFATrace≃SnocNFATrace w) = NFATrace→SnocNFATrace
    -- Iso.inv (NFATrace≃SnocNFATrace w) = SnocNFATrace→NFATrace
    -- Iso.rightInv (NFATrace≃SnocNFATrace w) b = {!!}
    -- Iso.leftInv (NFATrace≃SnocNFATrace w) a = {!!}

...
module DFADefs ℓ (Σ₀ : hSet ℓ) where
  ...
  acc? : (D : DFA) → D .Q .fst → Grammar
  acc? D q = DecProp-grammar (D .isAcc q)

  data DFATrace (D : DFA) (q : D .Q .fst) : String → Type ℓ where
    nil : ParseTransformer (acc? D q) (DFATrace D q) → DFATrace D q []
    cons : ∀ {c} →
      ParseTransformer {!!} {!!} →
      {!!}

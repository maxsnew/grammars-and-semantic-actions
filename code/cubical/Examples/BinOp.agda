...
module Automaton where
  ...
  -- the stack is a number, the number of open parens that are closed by the term
  ...
  data Opening where
    ...
    num  : ∀ {n b} → (LinΣ[ m ∈ ℕ ] literal (num m)) ⊗ Closing n b ⊢ Opening n b
    unexpected : ∀ {n} → ε ⊕ (literal RP ⊕ literal *) ⊗ ⊤ ⊢ Opening n false

  data Closing where
    rightMore : ∀ {n b} → literal RP ⊗ ((ε ⊕ literal RP ⊗ ⊤) & Closing n b) ⊢ Closing (suc n) b
    rightDone : ∀ {n b} → literal RP ⊗ ((literal * ⊗ ⊤) & Multiplying n b) ⊢ Closing (suc n) b
    rightUnmatched : literal RP ⊗ ⊤ ⊢ Closing 0 false
    rightUnexpectedLookahead : ∀ {n} → literal RP ⊗ literal LP ⊗ ⊤ ⊢ Closing n false

  data Multiplying where
    times : ∀ {n b} → literal * ⊗ Opening n b ⊢ Multiplying n b
    unexpected : ∀ {n} → ε ⊕ (literal LP ⊕ literal RP ⊕ anyNum) ⊗ ⊤ ⊢ Multiplying n false
  ...
  record Algebra ℓ : Type (ℓ-suc ℓ) where
    field
      ...
      [rightMore] : ∀ {n} → literal RP ⊗ ((ε ⊕ literal RP ⊗ ⊤) & UC n) ⊢ UC (suc n)
  ...
  --   left     : ∀ {n b} → literal LP ⊗ Exp (suc n) b ⊢ Exp n b -- this is wrong, need more states :/
  --   num      : ∀ {n b} → (LinΣ[ m ∈ ℕ ] literal (num m)) ⊗ Suffix n b ⊢ Exp n b
  ...
  --   rightMore : ∀ {n b} → literal RP ⊗ ((ε ⊕ literal RP ⊗ ⊤) & Exp n b) ⊢ Exp (suc n) b
  --   -- if the next character is a *, we start parsing a suffix
  --   rightDone : ∀ {n b} → literal RP ⊗ ((literal * ⊗ ⊤) & Suffix n b) ⊢ Exp (suc n) b
  ...
  --   rightLookAhead : ∀ {n} → (literal RP ⊕ ) ⊗ literal LP ⊗ ⊤ ⊢ Exp (suc n) false
  -- data Suffix where
...
module Soundness where
  ...
  buildExp = {!!} where
    ...
    Stk = Nat.elim ε-grammar (λ _ Stk⟨n⟩ → literal RP ⊗ KL* (literal * ⊗ Atom) ⊗ Stk⟨n⟩)
    alg : Automaton.Algebra ℓ-zero
    alg .UO n = Exp ⊗ Stk n
    alg .UC n = Stk n
    alg .UM n = literal * ⊗ Atom ⊗ KL* (literal * ⊗ Atom) ⊗ Stk n
    alg .[left] = ⊗-assoc ∘g ⟜3⊗ LL⟨1⟩.parens'
    alg .[num] = LinΣ-elim (λ _ → Atom.num ,⊗ nil ∘g ⊗-unit-r⁻) ,⊗ id -- LinΣ-elim (λ _ → Atom.num) ,⊗ id
    alg .[rightMore] = id ,⊗ (⟜0⊗ nil ∘g &-π₂) -- nil ,⊗ id ,⊗ &-π₂ ∘g ⊗-unit-l⁻
...
-- kind of would want a truly dependent linear type here to formulate it that way...

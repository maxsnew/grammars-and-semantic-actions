...
\newcommand{\nat}{\mathbb{N}}
\newcommand{\theoryname}{Grammar Theoretic Type Theory\xspace}
\newcommand{\theoryabbv}{GTTT\xspace}
...
\begin{abstract}
  In this paper we propose \theoryname~(\theoryabbv), a new syntactic formalism for formal grammars: a
  ...
  Internal to \theoryname, we carry out many classical constructions from
  ...
  We give a semantics in the category of grammars and prove a
...
developments: Chomsky's hierarchy of grammars \cite{chomThreeModels1956}, practical
...
\steven{I don't like the name \theoryname (\theoryabbv). Feel free to play with it though, as
  its set up as a pair of macros}
...
include one to prove a metatheoretic canonicity theorem. Finally, in
...
a bijective translation of parses. In other words, two grammars are isomorhpic
...
Upon inspection of the above definition, $\Grammar$ may equivalently be described as the functor
category $\Set^{\String}$ --- where the set $\String$ is viewed as a
discrete\footnote{Objects in $\String$ comprise the set of all strings, and the
  only morphisms are the identity morphisms.}
category. In fact, in this paper we will take $\Grammar$ to be defined as the
functor category $\Set^{\String}$. Such functor categories into $\Set$ --- often called
...
setting for formal grammar theory. \theoryabbv axiomatizes the necessary structure underlying $\Set^{\String}$ to
...
\cite{krishnaswami_integrating_2015}. The treatment of the non-linear types is
pretty standard, so below we focus on the linear syntax.
...
connective is exactly what is suggested by the syntax, ude in
large part to the inspiration taken from the grammar interpretation
when designing \theoryabbv.
...
derivative, we must include the equalities in \cref{fig:brzozowskideriv} as axioms.
...
Under the Curry-Howard-Lambek correspondence, the programs of the simply-typed
$\lambda$-calculus may be viewed under any one of the three equivalent views: as
terms in a simple functional language, as proofs in an intuitionisitc
propositional logic, or as morphisms in a Cartesian closed category. In the
same vein, with the syntax provided, we aim to give a
categorical accounting of the semantics of \theoryabbv. Let us begin by paring back
the connectives in the language and only considering the subtheory induced by
regular expressions.

\steven{Added this transition to handle putting the categorical semantics after
  the syntax. I think it sounds weird though}

By giving algebraic laws of manipulation and equivalence to regular expressions,
...
Returning to the problem of parsing, the output of a parse usually isn't the
...
of \typeabbv that carries a proof of canonicity along with it.
...
intrinsically verified and executable decision procedure for DFAs in \thoeryabbv
embedded in Agda.
...
like the head of the tape. We should reflect on the generality of our syntax, as
it allows a natural encoding of Turing machines.

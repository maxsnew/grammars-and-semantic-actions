...
structure WeakEquiv (A B : Grammar Alphabet) where
  to : A ⊢ B
  from' : B ⊢ A

scoped infixl:25 " ≈g " => WeakEquiv

def WeakEquiv.refl (A : Grammar Alphabet) : A ≈g A :=
  ⟨gId A, gId A⟩

def WeakEquiv.symm {A B : Grammar Alphabet} (e : A ≈g B) : B ≈g A :=
  ⟨e.from', e.to⟩

def WeakEquiv.trans {A B C : Grammar Alphabet} (e₁ : A ≈g B) (e₂ : B ≈g C) : A ≈g C :=
...
structure StrongEquiv (A B : Grammar Alphabet) where
  to : A ⊢ B
  from' : B ⊢ A
...
def StrongEquiv.refl (A : Grammar Alphabet) : A ≅g A :=
  ⟨gId A, gId A, rfl, rfl⟩

def StrongEquiv.symm {A B : Grammar Alphabet} (e : A ≅g B) : B ≅g A :=
  ⟨e.from', e.to, e.from_to, e.to_from⟩

def StrongEquiv.trans {A B C : Grammar Alphabet} (e₁ : A ≅g B) (e₂ : B ≅g C) : A ≅g C :=
  ⟨e₂.to ∘g e₁.to, e₁.from' ∘g e₂.from',
...
def StrongEquiv.toWeakEquiv {A B : Grammar Alphabet} (e : A ≅g B) : A ≈g B :=
  ⟨e.to, e.from'⟩

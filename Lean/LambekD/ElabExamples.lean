...
-- Identity: A ⊢ A
def idMorph (A : Grammar Paren) : A ⊢ A :=
  [| x => x |]

-- Tensor intro: A ⊗ B ⊢ A ⊗ B (identity on tensor, single pattern)
def tensorId (A B : Grammar Paren) : A ⊗ B ⊢ A ⊗ B :=
  [| x => x |]

-- Tensor elimination + rebuild: A ⊗ B ⊢ A ⊗ B (multi-pattern)
def tensorLetId (A B : Grammar Paren) : A ⊗ B ⊢ A ⊗ B :=
  [| a b => (a, b) |]
...
def evalR (A B : Grammar Paren) : A ⊗ (A ⊸ B) ⊢ B :=
  [| a f => f a |]

-- Evaluation: (B ⟜ A) ⊗ A ⊢ B  (function LEFT of argument → left-app)
def evalL (A B : Grammar Paren) : (B ⟜ A) ⊗ A ⊢ B :=
  [| f a => f a |]
...
def injLeftEx (A B : Grammar Paren) : A ⊢ A ⊕ B :=
  [| x => inl x |]

-- Right injection: B ⊢ A ⊕ B
def injRightEx (A B : Grammar Paren) : B ⊢ A ⊕ B :=
  [| x => inr x |]
...
-- Pair: A ⊢ A & A (diagonal)
def diag (A : Grammar Paren) : A ⊢ A & A :=
  [| x => ⟨x, x⟩ |]

-- First projection: A & B ⊢ A
def proj1 (A B : Grammar Paren) : A & B ⊢ A :=
  [| x => fst x |]

-- Second projection: A & B ⊢ B
def proj2 (A B : Grammar Paren) : A & B ⊢ B :=
  [| x => snd x |]
...
def assocR (A B C : Grammar Paren) : (A ⊗ B) ⊗ C ⊢ A ⊗ (B ⊗ C) :=
  [| ab c => let (a, b) = ab in (a, (b, c)) |]

-- Reassociate the other way: A ⊗ (B ⊗ C) ⊢ (A ⊗ B) ⊗ C
def assocL (A B C : Grammar Paren) : A ⊗ (B ⊗ C) ⊢ (A ⊗ B) ⊗ C :=
...
def curryR (A B C : Grammar Paren) : (A ⊗ B) ⊸ C ⊢ B ⊸ (A ⊸ C) :=
...
def myMorph (A : Grammar Paren) : A ⊢ A := fun _ a => a

def escapedId (A : Grammar Paren) : A ⊢ A :=
  [| x => #[myMorph A] x |]

-- Chain two morphisms
def escapedChain (A B C : Grammar Paren) (f : A ⊢ B) (g : B ⊢ C) : A ⊢ C :=
  [| x => #[g] (#[f] x) |]
...
def idParam (X : Type) (F : X → Grammar Paren) (x : X) : F x ⊢ F x :=
  [| v => v |]
...
def idxProdConst (X : Type) (A : Grammar Paren) : A ⊢ &[x ∈ X] A :=
  [| v => Λ (x : X) => v |]

-- Indexed product elim (projection): &[x ∈ Bool] F x ⊢ F true
def idxProdProj (F : Bool → Grammar Paren) : &[x ∈ Bool] F x ⊢ F true :=
  [| v => v ⌈true⌉ |]

-- Indexed coproduct intro: F true ⊢ ⊕[x ∈ Bool] F x
def idxCoprodInj (F : Bool → Grammar Paren) : F true ⊢ ⊕[x ∈ Bool] F x :=
  [| v => σ⟨true, v⟩ |]

-- Indexed coproduct elim: ⊕[x ∈ Bool] F x ⊢ ⊕[x ∈ Bool] F x
def idxCoprodCase (F : Bool → Grammar Paren) : ⊕[x ∈ Bool] F x ⊢ ⊕[x ∈ Bool] F x :=
...
def sorryEx (A B : Grammar Paren) : A ⊢ B := [| x => sorry |]
def sorryLet (A B C : Grammar Paren) : A ⊗ B ⊢ C := [| a b => sorry |]
...
def prodPatFst (A B : Grammar Paren) : A & B ⊢ A := [| ⟨a, b⟩ => a |]

-- Second projection via product pattern
def prodPatSnd (A B : Grammar Paren) : A & B ⊢ B := [| ⟨a, b⟩ => b |]

-- Product pattern identity
def prodPatId (A B : Grammar Paren) : A & B ⊢ A & B := [| ⟨a, b⟩ => ⟨a, b⟩ |]

-- Product pattern with tensor
def prodTensor (A B C : Grammar Paren) : (A & B) ⊗ C ⊢ A ⊗ C := [| ⟨a, b⟩ c => (a, c) |]
...
def starConsLet (A : Grammar Paren) : A ⊗ StarG A ⊢ StarG A :=
